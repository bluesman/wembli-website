    ...
    return {
        ...
        controller: [ "$scope", "$element", "$attrs", "$transclude", function($scope) {
            $scope.stepCompleted = {
                ...
                $scope.currentStep = "nav-" + step, $scope.showStep = step;
                ...
                return $scope.customer.email ? ($location.hash("#section1"), void $location.path("/plan")) : ($scope.signup.noContinue = !0, 
                $scope.gotoStep("step1"));
            }, plan.get(function(p) {
                if ($scope.plan = p, $scope.loggedIn = loggedIn.check(), !$scope.plan || "undefined" != typeof $scope.plan.event.eventId) {
                    var hash = $location.hash(), initialStep = /^step/.test(hash) ? hash : "step1";
                    "_=_" === hash && (initialStep = "step2"), $scope.step1 = {}, customer.get() && Object.keys(customer.get()).length > 0 ? $scope.customer = customer.get() : ($scope.customer = {}, 
        ...
        compile: function() {
            return function(scope, element) {
                plan.get(function(p) {
                    ...
                    "undefined" != typeof p.rsvpDate && (defaultDate = new Date(p.rsvpDate)), element.pikaday({
                        bound: !1,
                        minDate: startDate,
                        maxDate: endDate,
    ...
    return {
        ...
        controller: [ "$scope", "$element", "$attrs", "$transclude", function($scope) {
            $scope.listId = "a55323395c", $scope.$on("forgot-password-email-sent", function(e, err) {
                ...
                }), $scope.customer && $scope.customer.email ? ($scope.navData["nav-step1"] = $scope.customer.email, 
                $scope.stepCompleted["nav-step1"] = !0, $scope.showSignupView = !0) : $scope.showSignupForm = !0;
...
} ]).directive("invitationWizardStep2", [ "wembliRpc", "$window", "$filter", function(wembliRpc, $window, $filter) {
    return {
        restrict: "C",
        controller: [ "$scope", "$element", "$attrs", "$transclude", function($scope) {
            $scope.plan && $scope.plan.rsvpDate && ($scope.navData["nav-step2"] = $filter("date")($scope.plan.rsvpDate, "mediumDate"), 
            $scope.stepCompleted["nav-step2"] = !0), $scope.$on("plan-rsvp-changed", function(e, date) {
