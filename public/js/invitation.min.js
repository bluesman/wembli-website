...
angular.module("wembliApp.directives.invitationWizard", []).directive("inviteFriendsWizard", [ "$rootScope", "$http", "$filter", "$window", "$timeout", "plan", "$location", "wembliRpc", "customer", "facebook", "twitter", "loggedIn", "overlay", function($rootScope, $http, $filter, $window, $timeout, plan, $location, wembliRpc, customer, facebook, twitter, loggedIn, overlay) {
    return {
        restrict: "C",
        controller: [ "$scope", "$element", "$attrs", "$transclude", function($scope) {
            $scope.stepCompleted = {
                ...
                $scope.plan = p, $scope.loggedIn = loggedIn.check();
                ...
                customer.set($scope.customer)), $scope.gotoStep(initialStep), overlay.loading(!1), 
                overlay.hide();
            });
        } ],
        compile: function() {
            ...
            $scope.listId = "a55323395c", $scope.$on("forgot-password-email-sent", function(e, err) {
                ...
                if ($scope.forgotPasswordEmailError = !1, $scope.forgotPasswordEmailSent = !1, $scope.signup.$valid) {
                    ...
                    wembliRpc.fetch("invite-friends.submit-signup", rpcArgs, function(err, result) {
                        if (result.exists && !result.noPassword) return $scope.login.accountExists = !0, 
                ...
                if ($scope.forgotPasswordEmailError = !1, $scope.forgotPasswordEmailSent = !1, $scope.login.$valid) {
                    ...
                    wembliRpc.fetch("invite-friends.submit-login", rpcArgs, function(err, result) {
                        return result.noPassword ? $scope.showForm("showLoginUnconfirmedForm", "showLoginForm") : result.invalidCredentials ? void ($scope.login.invalidCredentials = !0) : ($scope.navData["nav-step1"] = $scope.customer.email, 
        ...
        controller: [ "$scope", "$element", "$attrs", "$transclude", function($scope) {
            $scope.submitRsvp = function() {
                ...
                wembliRpc.fetch("invite-friends.submit-rsvp", rpcArgs, function(err, result) {
                    return result.noCustomer ? ($scope.signup.noContinue = !0, $scope.gotoStep("step1")) : $scope.gotoStep("step3");
                });
...
} ]).directive("invitationWizardStep3", [ "wembliRpc", "$window", "facebook", "plan", "$http", "$rootScope", "$filter", function(wembliRpc, $window, facebook, plan, $http, $rootScope, $filter) {
    return {
        restrict: "C",
        controller: [ "$scope", "$element", "$attrs", "$transclude", function($scope) {
            $scope.navData["nav-step3"] = 0, $scope.stepCompleted["nav-step3"] = !1, $scope.handleFriendsFetch = function() {
                ...
                if ("input" === $event.target.localName) {
                    ...
                    plan.addFriend(addFriendArgs, function(err, result) {
                        return result.noCustomer ? ($scope.signup.noContinue = !0, $scope.gotoStep("step1")) : void ($event.currentTarget.control.checked && facebook.feedDialog({
                            ...
                            null === response ? $scope.handleFriendsFetch() : (console.log(result), $http.get("/callback/facebook/rsvp/" + $scope.plan.guid + "/" + result.friend.inviteStatusConfirmation.token).success(function() {
                                plan.fetch(function() {
...
} ]).directive("invitationWizardStep5", [ "wembliRpc", "$window", "plan", "$timeout", "$rootScope", function(wembliRpc, $window, plan) {
    return {
        restrict: "C",
        controller: [ "$scope", "$element", "$attrs", "$transclude", function($scope) {
            $scope.navData["nav-step5"] = 0, $scope.stepCompleted["nav-step5"] = !1;
            var n = [];
            plan.get(function() {
                var friends = plan.getFriends();
                angular.forEach(friends, function(friend) {
                    friend.checked = friend.inviteStatus, "wemblimail" === friend.contactInfo.service && (n.push(friend), 
                    $scope.navData["nav-step5"]++);
                }), $scope.invitedFriends = n;
            }), $scope.sendWemblimail = function() {
                if ($scope.wemblimailForm.$valid) {
                    $scope.stepCompleted["nav-step5"] = !0;
                    var addFriendArgs = {
                    ...
                    wembliRpc.fetch("invite-friends.sendWemblimail", addFriendArgs, function(err, result) {
                        ...
                        friend.checked = friend.inviteStatus, $scope.invitedFriends.unshift(friend), $scope.wemblimail.lastSentEmail = $scope.wemblimail.email, 
