...
} ]), angular.module("wembliApp.controllers.tickets", []).controller("TicketsCtrl", [ "$scope", "wembliRpc", "plan", "customer", "ticketPurchaseUrls", "tnConfig", "tuMap", "$location", "$window", "$timeout", function($scope, wembliRpc, plan, customer, ticketPurchaseUrls, tnConfig, tuMap, $location, $window) {
    $scope.tnUrl = ticketPurchaseUrls.tn, $scope.distinctSplits = [ 1 ], $scope.notFound = !1, 
    $scope.minTixPrice = 0, $scope.maxTixPrice = 200, $scope.handlePriceRange = function() {
        ...
        "friend" === $scope.context && ($scope.backToPlan = !0), wembliRpc.fetch("event.getTickets", {
            ...
            angular.forEach(result.tickets, function(el) {
                ...
                el.ticketsInPlan = !1, angular.forEach(plan.getTickets(), function(ticketInPlan) {
                    el.ticketsInPlan = !0, el._id = ticketInPlan._id;
