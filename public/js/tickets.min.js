...
!function($, undefined) {
...
} ]), angular.module("wembliApp.controllers.tickets", []).controller("TicketsCtrl", [ "$scope", "wembliRpc", "plan", "customer", "ticketPurchaseUrls", "tnConfig", "tuMap", "$location", "$window", "$timeout", "$rootScope", "overlay", "googleAnalytics", function($scope, wembliRpc, plan, customer, ticketPurchaseUrls, tnConfig, tuMap, $location, $window, $timeout, $rootScope, overlay, googleAnalytics) {
    $scope.tnUrl = ticketPurchaseUrls.tn, $scope.distinctSplits = [ 1 ], $scope.notFound = !1, 
    $scope.minTixPrice = 0, $scope.maxTixPrice = 200, $scope.ticketsChosen = !1, $scope.handlePriceRange = function() {
        ...
        $scope.nextText = $scope.backToPlan ? "Back To Plan Dashboard" : "Continue To Plan Preferences", 
        $scope.buyTicket = function(idx) {
            $scope.currentTicketIdx = idx;
            var ticket = $scope.tickets[idx];
            $scope.selectedQty = ticket.selectedQty, delete ticket.$$hashKey;
            var ticketGroup = {
                service: "tn",
            ...
            console.log("add ticketgroup"), console.log(ticketGroup), plan.addTicketGroup(ticketGroup, function(err, results) {
                ...
                $scope.tickets[idx] = ticket, "split-first" === $scope.plan.preferences.payment) {
                    for (var sections = $("#venue-map-container").tuMap("GetSections"), i = sections.length - 1; i >= 0; i--) if (sections[i].Name === ticketGroup.ticketGroup.Section) {
                        $scope.activeSection = sections[i];
                        break;
                    }
                    $scope.ticketsConfirm = !0, overlay.show();
                } else {
                    $timeout(function() {
        ...
        wembliRpc.fetch("event.getTickets", {
            ...
            }, $("#tickets-list > div").css("height", $($window).height() - 172), tuMap.init("#venue-map-container", options), 
            $(".price-slider").length) {
                $(".price-slider").slider({
                    range: !0,
    ...
    }, $scope.submitForm = function() {}, $scope.cancelForm = function() {
