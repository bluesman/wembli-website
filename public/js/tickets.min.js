...
!function($, undefined) {
...
}(jQuery), angular.module("wembliApp", [ "ngRoute", "ngAnimate", "mgcrea.ngStrap", "wembliApp.controllers", "wembliApp.controllers.tickets", "wembliApp.controllers.header", "wembliApp.filters", "wembliApp.services", "wembliApp.services.header", "wembliApp.services.facebook", "wembliApp.services.twitter", "wembliApp.services.pixel", "wembliApp.services.venueMap", "wembliApp.directives", "wembliApp.directives.header", "wembliApp.directives.venueMap" ]).config([ "$routeProvider", "$locationProvider", function() {} ]).run([ "$timeout", "initRootScope", "$rootScope", "$location", "$route", "$window", "facebook", "twitter", "plan", "wembliRpc", "overlay", function($timeout, initRootScope, $scope, $location, $route, $window, facebook, twitter, plan, wembliRpc, overlay) {
    console.log("run event-options angular app"), overlay.show(), $timeout(function() {
        plan.fetch(function() {});
    }), $window.fbAsyncInit = function() {
        facebook.getLoginStatus(), $scope.facebook = facebook;
    }, twitter.getLoginStatus();
} ]), angular.module("wembliApp.controllers.tickets", []).controller("TicketsCtrl", [ "$scope", "wembliRpc", "plan", "customer", "ticketPurchaseUrls", "tnConfig", "tuMap", "$location", "$window", "$timeout", "$rootScope", "overlay", "googleAnalytics", "header", function($scope, wembliRpc, plan, customer, ticketPurchaseUrls, tnConfig, tuMap, $location, $window, $timeout, $rootScope, overlay, googleAnalytics, header) {
    header.fixed(), $scope.tnUrl = ticketPurchaseUrls.tn, $scope.distinctSplits = [ 1 ], 
    $scope.notFound = !1, $scope.minTixPrice = 0, $scope.maxTixPrice = 200, $scope.ticketsChosen = !1, 
    $scope.handlePriceRange = function() {
        ...
        newVal !== oldVal && $("#venue-map-container").tuMap("Refresh", "ProcessTickets");
    }), $scope.$watch("currentTicket.selectedQty", function(newVal) {
        if (console.log("currentTicket selectedqty changed"), console.log(newVal), newVal && ($scope.currentTicket = $scope.tickets[$scope.currentTicketIdx], 
        $scope.currentTicket)) {
            $scope.currentTicket.selectedQty = parseInt(newVal);
            var shipping = 15, serviceCharge = .15 * parseFloat($scope.currentTicket.ActualPrice) * parseInt($scope.currentTicket.selectedQty), actualPrice = parseFloat($scope.currentTicket.ActualPrice) * parseInt($scope.currentTicket.selectedQty), amountPaid = parseFloat(actualPrice) + parseFloat(serviceCharge) + parseFloat(shipping);
            $scope.currentTicket.amountPaid = amountPaid.toFixed(2);
        }
    }), $scope.$watch("confirmPriceQty", function(newVal) {
        $("#bought-tickets-button").prop("disabled", !newVal);
    }), plan.get(function(p) {
        $scope.backToPlan = !1, null !== p.organizer.rsvp.decision && ($scope.backToPlan = !0), 
        ...
        $scope.buyTicket = function(idx) {
            ...
            $scope.currentTicket = ticket, $scope.currentTicket.selectedQty = ticket.selectedQty || ticket.maxSplit, 
            $scope.currentTicket.selectedQty = $scope.currentTicket.selectedQty.toString(), 
            console.log($scope.currentTicket);
            for (var sections = $("#venue-map-container").tuMap("GetSections"), i = sections.length - 1; i >= 0; i--) if (sections[i].Name === ticket.Section) {
                $scope.activeSection = sections[i], ticket.url = $scope.activeSection.SectionViewUrl;
                break;
            }
            ...
            console.log("add ticketgroup"), console.log(ticketGroup), plan.addTicketGroup(ticketGroup, function(err, results) {
                if (console.log(results), ticket.ticketsInPlan = !0, results.ticketGroup._id && (ticket._id = results.ticketGroup._id), 
                $scope.tickets[idx] = ticket, "split-first" === $scope.plan.preferences.payment) $scope.ticketsConfirm = !0, 
                overlay.show(); else {
                    $timeout(function() {
                        $rootScope.$apply(function() {
                            console.log("showbuytixoffsite"), $scope.buyTicketsOffsite = !0, overlay.show();
                        });
                    }, 1500);
                }
            });
        }, $scope.removeTicketGroup = function(idx) {
            var index = "undefined" != typeof idx ? idx : $scope.currentTicketIdx;
            if ("undefined" == typeof index) return $scope.buyTicketsOffsite = !1, $scope.ticketsConfirm = !1, 
            void overlay.hide();
            var id = $scope.tickets[index]._id ? $scope.tickets[index]._id : $scope.tickets[index].ID;
            console.log("removing ticket id: " + id), plan.removeTicketGroup({
                ticketId: id
            }, function() {
                $scope.tickets[index].ticketsInPlan = !1, $scope.currentTicketIdx && delete $scope.currentTicketIdx, 
                $scope.currentTicket && delete $scope.currentTicket, $scope.buyTicketsOffsite && ($scope.buyTicketsOffsite = !1, 
                overlay.hide()), $scope.ticketsConfirm && ($scope.ticketsConfirm = !1, overlay.hide());
            });
        }, $scope.boughtTickets = function() {
            if (console.log("set receipt for tix with idx: " + $scope.currentTicketIdx), "undefined" == typeof $scope.currentTicketIdx) return console.log("no currenttixid"), 
            void ($window.location.href = $scope.nextLink);
            ...
            plan.addTicketGroupReceipt({
                ...
                console.log("receipt added"), console.log(err, result), googleAnalytics.trackEvent("Plan", "boughtTickets", $scope.plan.event.eventName, "", function() {
                    console.log("going to next page: " + $scope.nextLink), $window.location.href = $scope.nextLink;
                });
            });
        }, $rootScope.$on("overlay-clicked", function() {
            $scope.removeTicketGroup(), console.log("overlay clicked");
        ...
        wembliRpc.fetch("event.getTickets", {
            ...
            "undefined" == typeof result.tickets[0] && ($scope.notFound = !0);
            var filteredTicketsList = [];
            angular.forEach(result.tickets, function(el) {
                /parking/gi.test(el.Section) || (el.sessionId = tnConfig.generateSessionId(), parseInt(el.ActualPrice) < $scope.minTixPrice && ($scope.minTixPrice = parseInt(el.ActualPrice)), 
                parseInt(el.ActualPrice) > $scope.maxTixPrice && ($scope.maxTixPrice = parseInt(el.ActualPrice)), 
                el.selectedQty = el.ValidSplits.int[0], el.maxSplit = 1, "string" == typeof el.ValidSplits.int && (el.ValidSplits.int = [ el.ValidSplits.int ]), 
                el.ValidSplits.int.sort(function(a, b) {
                    return a - b;
                }), el.maxSplit = el.ValidSplits.int[el.ValidSplits.int.length - 1], el.maxSplit > $scope.maxSplit && ($scope.maxSplit = el.maxSplit), 
                $location.search().qty && el.ValidSplits.int.indexOf($location.search().qty) && (el.selectedQty = $location.search().qty), 
                el.ticketsInPlan = !1, angular.forEach(plan.getTickets(), function(ticketInPlan) {
                    ticketInPlan.ticketGroup.ID === el.ID && (el.ticketsInPlan = !0, $scope.ticketsChosen = !0, 
                    "undefined" != typeof ticketInPlan._id && (el._id = ticketInPlan._id), "undefined" != typeof ticketInPlan.purchased && (el.purchased = ticketInPlan.purchased));
                }), filteredTicketsList.push(el));
            }), $scope.tickets = filteredTicketsList, $scope.sortByPrice();
            ...
            if (options.MapId = $scope.plan.event.data.VenueConfigurationID, options.SingleSectionSelection = !0, 
            options.ServiceUrl = "//imap.ticketutils.net", options.OnInit = function(e, data) {
                $("#venue-map-loading").hide(), overlay.hide(), console.log(data), tuMap.mapType = data.MapType, 
                "Interactive" === tuMap.mapType ? /ga_venue_tn/.test($scope.plan.event.data.MapUrl) ? tuMap.displayStaticMap($scope.plan.event.data.MapUrl) : ($(".ZoomControls").appendTo("#venue-map-row"), 
                $(".ZoomControls + div").appendTo("#venue-map-row")) : tuMap.displayStaticMap($scope.plan.event.data.MapUrl);
            ...
            }, $("#tickets-list > div").css("height", $($window).height() - 191), tuMap.init("#venue-map-container", options), 
            $(".price-slider").length) {
                $(".price-slider").slider({
                    range: !0,
                    ...
                    stop: function() {
                        var q = $(this).val();
                        console.log("quantity in price slider stop: " + q), filterTickets({
                            quantity: q
                        });
                    }
                });
                ...
                scope.$watch("tickets", function(newVal, oldVal) {
                    ...
                    console.log("get plan"), wembliRpc.fetch("event.getTickets", {
                        ...
                        options.OnInit = function(e, MapType) {
                            console.log(MapType), console.log("move zoom controls"), "Interactive" == MapType ? /ga_venue_tn/.test(scope.event.MapUrl) ? initStaticMap() : (console.log("moving zoom controls"), 
                            $(".ZoomControls").appendTo("#venue-map")) : initStaticMap();
                        }, options.OnError = function(e, Error) {
                            ...
                            Group.Selected;
                        }, options.OnTicketSelected = function() {}, options.OnReset = function() {}, $("#venue-map-container").tuMap(options), 
                        $(".price-slider").length) {
                            $(".price-slider").slider({
                                range: !0,
                                min: scope.minTixPrice,
    ...
    return self.options = {
        ...
        displayStaticMap: function(mapUrl) {
            ...
            var img = new Image();
            img.src = mapUrl, img.onload = function() {
                var w = img.width, h = img.height;
                $(this.container).css("width", $($window).width() - 680), $(this.container).css("position", "relative"), 
                $(this.container).empty().css("overflow-y", "auto").css("float", "right").prepend('<div id="static-map-image" style="overflow-y:auto;background:transparent url(\'' + mapUrl + "') no-repeat center center;margin:auto;padding:20px;height:" + h + "px;width:" + w + 'px;" />');
