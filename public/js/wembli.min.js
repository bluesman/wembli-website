...
!function(window, undefined) {
    ...
    function minErr(module) {
        return function() {
            ...
            }), message = message + "\nhttp://errors.angularjs.org/1.2.22-build.364+sha.29eaabc/" + (module ? module + "/" : "") + code, 
    ...
    var csp = function() {
        ...
        full: "1.2.22-build.364+sha.29eaabc",
        major: 1,
        minor: 2,
        dot: 22,
        codeName: "snapshot"
            ...
            link: function(scope, element, attr, controller) {
                ...
                options = datepicker.$options, angular.forEach([ "minDate", "maxDate" ], function(key) {
                    angular.isDefined(attr[key]) && attr.$observe(key, function(newValue) {
                        ...
                        } else datepicker.$options[key] = angular.isString(newValue) && newValue.match(/^".+"$/) ? +new Date(newValue.substr(1, newValue.length - 2)) : isNumeric(newValue) ? +new Date(parseInt(newValue, 10)) : +new Date(newValue);
        ...
        this.$get = [ "$window", "$rootScope", "$tooltip", function($window, $rootScope, $tooltip) {
            function DropdownFactory(element, config) {
                ...
                return $dropdown.hide = function() {
                    ...
                    $dropdown.$element.parent().toggleClass("open"), hide();
                }, $dropdown;
            }
        ...
        this.$get = [ "$window", "$rootScope", "$compile", "$q", "$templateCache", "$http", "$animate", "dimensions", "$$rAF", function($window, $rootScope, $compile, $q, $templateCache, $http, $animate, dimensions, $$rAF) {
            function TooltipFactory(element, config) {
                ...
                return $tooltip.$promise.then(function(template) {
                    ...
                    return $tooltip.$isShown ? (scope.$emit(options.prefixEvent + ".hide.before", $tooltip), 
        ...
        this.$get = [ "$window", "$rootScope", "$tooltip", function($window, $rootScope, $tooltip) {
            function TypeaheadFactory(element, controller, config) {
                ...
                scope.$resetMatches = function() {
                    ...
                    /(38|40|13)/.test(evt.keyCode) && (evt.preventDefault(), $typeahead.$isVisible() && evt.stopPropagation(), 
    ...
    return {
        ...
        link: function(scope, element, attr) {
            scope.$watch(attr.level, function(pl) {
                ...
                if (!len && (console.log("pl is:"), console.log(pl), "undefined" != typeof pl)) {
                    var d = parseInt(pl);
                    console.log("d:" + d);
                    for (var i = d; i > 0; i--) console.log("print dollar"), element.append('<i class="fa fa-usd"/>');
                }
            });
        }
    };
    ...
    return {
        ...
        link: function(scope, element, attr) {
            ...
            scope.$watch(attr.rating, function(rating) {
                var len = element.children().length;
                if (!len && (console.log(rating), "undefined" != typeof rating)) {
                    for (var r = parseFloat(rating), floor = parseInt(Math.floor(r)), ceil = parseInt(Math.ceil(r)), i = floor; i > 0; i--) element.append('<i class="fa fa-star yellow"/>');
                    r - floor > 0 && element.append('<i class="fa fa-star-half-o yellow"/>');
                    for (var empty = totalStars - ceil, i = empty; i > 0; i--) element.append('<i class="fa fa-star-o yellow"/>');
                }
            });
        }
    };
...
angular.module("wembliApp.services", []).factory("overlay", [ "$rootScope", function() {
    return {
        show: function() {
    ...
    self.fetchStack = 0, {
        ...
        fetch: function(args, callback) {
            if ("undefined" == typeof callback && (callback = args, args = {}), self.fetchInProgress) {
                ...
                console.log("set customer:"), console.log(customer.get())), self.fetchInProgress = !1, 
                $rootScope.$broadcast("plan-fetched", {}), callback && callback(self);
            });
        },
    ...
    return {
        init: function(header, scroll) {
            ...
            self.scroll = scroll, self.prevScroll = angular.element(scroll).scrollTop(), angular.element(header).show(), 
        ...
        fixed: function() {
            console.log("turn off header scroll handler: " + self.scroll), angular.element(self.scroll).off("scroll");
        }
    };
