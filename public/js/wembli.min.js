...
function(window, angular) {
    ...
    Modal.prototype = {
        ...
        hide: function(e) {
            ...
            void ($.support.transition && this.$element.hasClass("fade") ? this.hideWithTransition() : this.hideModal()));
        },
        layout: function() {
        ...
        teardown: function() {
            ...
            this.$element.off(".modal"), this.$element.removeData("modal"), void this.$element.removeClass("in").attr("aria-hidden", !0)) : (this.$element.remove(), 
            void (this.$element = null));
        }
        ...
        $provide.decorator("$animate", [ "$delegate", "$injector", "$sniffer", "$rootElement", "$$asyncQueueBuffer", "$rootScope", "$document", function($delegate, $injector, $sniffer, $rootElement, $$asyncQueueBuffer, $rootScope) {
            ...
            function performAnimation(animationEvent, className, element, parentElement, afterElement, domOperation, doneCallback) {
                function onBeforeAnimationsComplete(cancelled) {
                    ...
                    fireDOMOperation(), cancelled === !0) return void closeAnimation();
                ...
                function invokeRegisteredAnimationFns(animations, phase, allAnimationFnsComplete) {
                    ...
                    forEach(animations, function(animation, index) {
                        ...
                        return "before" != phase || "enter" != animationEvent && "move" != animationEvent ? void (animation[phase] ? animation[endFnName] = setClassOperation ? animation[phase](element, classNameAdd, classNameRemove, animationPhaseCompleted) : isClassBased ? animation[phase](element, className, animationPhaseCompleted) : animation[phase](element, animationPhaseCompleted) : animationPhaseCompleted()) : void animationPhaseCompleted();
                    });
                }
        ...
        this.$get = [ "$window", "$document", "$rootScope", "$tooltip", function($window, $document, $rootScope, $tooltip) {
            function SelectFactory(element, controller, config) {
                ...
                scope.$activate = function(index) {
                    ...
                    $select.activate(index), controller.$setViewValue(options.multiple ? scope.$activeIndex.map(function(index) {
                        return scope.$matches[index].value;
                    }) : value), controller.$render(), parentScope && parentScope.$digest(), options.multiple || ("focus" === options.trigger ? element[0].blur() : $select.$isShown && $select.hide()), 
    ...
    var cc = {
        ...
        create: function(data, callback) {
            if (!data) return void noDataError(callback);
            if (!_marketplace_uri) return void noDataError(callback, "You need to call balanced.init first");
        ...
        lookupRoutingNumber: function(routingNumber, callback) {
            if (!_marketplace_uri) return void noDataError(callback, "You need to call balanced.init first");
            if (!routingNumber) return void noDataError(callback);
            ...
            receiveMessage: function(callback, origin) {
                if ("undefined" != typeof window) if (window.postMessage) {
                    var attachedCallback = function(response) {
                        return response.origin.toLowerCase() !== origin.toLowerCase() ? !1 : void callback(deserialize(response.data));
                    };
        ...
        compile: function() {
            return function(scope, element, attr) {
                ...
                attr.$observe("ngModel", function(m) {
                    ...
                    "undefined" != typeof newVal && ms.multiselect(newVal ? "enable" : "disable");
    ...
    return wembliRpc.fetch = function(method, args, callback, transformRequest, transformResponse) {
        ...
        var data = JSON.stringify(jsonRpc), cb = function(err, result) {
            ...
            if (err) {
                ...
                return void $rootScope.$broadcast(eventName + "callbackComplete", {
                    method: method
                });
            }
    ...
    return {
        init: function(header, scroll) {
            ...
            angular.element(scroll).on("scroll", function() {
                return angular.element(scroll).scrollTop() < 1 ? void angular.element(header).show() : angular.element(scroll).height() < angular.element(scroll).scrollTop() ? void angular.element(header).hide() : (angular.element(scroll).scrollTop() > self.prevScroll ? angular.element(header).is(":visible") && angular.element(header).fadeOut(250) : angular.element(header).is(":visible") || angular.element(header).fadeIn(250), 
                void (self.prevScroll = angular.element(scroll).scrollTop()));
            });
        },
        fixed: function() {}
    };
    ...
    return this.auth = null, this.selectedFriends = {}, this.friends = null, this.allFriends = null, 
    {
        tweet: function(args, cb) {
            wembliRpc.fetch("twitter.tweet", args, function(err, result) {
                return err ? cb(err) : ($rootScope.$broadcast("twitter-tweet", {}), void cb(err, result));
            });
        },
        getAuth: function() {
            return self.auth;
        ...
        getLoginStatus: function() {
            wembliRpc.fetch("twitter.getLoginStatus", {}, function(err, result) {
                return err ? !1 : (self.auth = result.loginStatus, void $rootScope.$broadcast("twitter-login-status", {
                    auth: self.auth
                }));
            });
        },
