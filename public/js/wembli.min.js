...
angular.module("wembliApp.services", []).factory("loadingModal", [ "$rootScope", function($rootScope) {
    ...
    self.fetchStack = 0, {
        ...
        rsvpComplete: function(f) {
            var complete = !0;
            if ("undefined" == typeof self.plan.rsvpDate) return f(!complete);
            var now = new Date();
            now.setHours(0, 0, 0, 0);
            var t1 = now.getTime(), rsvpDate = new Date(self.plan.rsvpDate);
            rsvpDate.setHours(24, 0, 0, 0);
            var t2 = rsvpDate.getTime();
            if (t1 >= t2) return f(complete);
            if (0 == self.friends.length) return f(!complete);
            for (var i = 0; i < self.friends.length; i++) if (null === self.friends[i].rsvp.decision) return f(!complete);
            return f(complete);
        },
        submitRsvpComplete: function(rsvpComplete, callback) {
            wembliRpc.fetch("plan.submitRsvpComplete", {
                rsvpComplete: rsvpComplete
            }, function(err, result) {
                return callback ? callback(err, result) : void 0;
            });
        },
        ponyUpSent: function() {
        ...
        acknowledgeNotification: function(key, callback) {
            wembliRpc.fetch("plan.acknowledgeNotification", {
                key: key
            }, function(err, result) {
                return callback ? callback(err, result) : void 0;
            });
