...
function createdAgoString(date, $filter) {
    ...
    now = parseInt(now / 1e3);
    var then = new Date(date), t = then.getTime();
    t = parseInt(t / 1e3);
    var s = parseInt(now - t), dateStr = "on " + $filter("date")(then, "shortDate") + ".";
    if (86400 >= s) {
        var dd = parseInt(s / 3600), sString = 1 == dd ? "" : "s";
        dateStr = dd + " hour" + sString + " ago.";
    }
    if (3600 >= s) {
        var m = parseInt(s / 60), sString = 1 == m ? "" : "s";
...
!function(global, factory) {
    ...
    function minErr(module) {
        return function() {
            ...
            }), message = message + "\nhttp://errors.angularjs.org/1.2.14-build.2317+sha.348a771/" + (module ? module + "/" : "") + code, 
    ...
    function LocationHashbangUrl(appBase, hashPrefix) {
        ...
        parseAbsoluteUrl(appBase, this, appBase), this.$$parse = function(url) {
            ...
            "" === withoutHashUrl && "?" === withoutBaseUrl.charAt(0) && (withoutHashUrl = withoutBaseUrl), 
            parseAppUrl(withoutHashUrl, this, appBase), this.$$path = removeWindowsDriveName(this.$$path, withoutHashUrl, appBase), 
            this.$$compose();
        }, this.$$compose = function() {
            var search = toKeyValue(this.$$search), hash = this.$$hash ? "#" + encodeUriSegment(this.$$hash) : "", url = "";
            this.$$url = encodePath(this.$$path) + (search ? "?" + search : "") + hash, this.$$url && (url = this.$$path ? hashPrefix + this.$$url : this.$$url), 
    ...
    function addNativeHtml5Validators(ctrl, validatorName, element) {
        ...
        if (isObject(validity)) {
            var validator = function(value) {
                return ctrl.$error[validatorName] || !(validity.badInput || validity.customError || validity.typeMismatch) || validity.valueMissing ? value : (ctrl.$setValidity(validatorName, !1), 
                void 0);
            };
    ...
    var SNAKE_CASE_REGEXP = /[A-Z]/g, version = {
        full: "1.2.14-build.2317+sha.348a771",
        major: 1,
        minor: 2,
        dot: 14,
        codeName: "snapshot"
...
function(window, angular) {
    ...
    function targetIsSelf(callback) {
        return function(e) {
            return this === e.target ? callback.apply(this, arguments) : void 0;
        };
    }
    ...
    ModalManager.prototype = {
        ...
        init: function(element, options) {
            if (this.$element = $(element), this.options = $.extend({}, $.fn.modalmanager.defaults, this.$element.data(), "object" == typeof options && options), 
            this.stack = [], this.backdropCount = 0, this.options.resize) {
                ...
                $(window).on("resize.modal", function() {
                    resizeTimeout && clearTimeout(resizeTimeout), resizeTimeout = setTimeout(function() {
                        for (var i = 0; i < that.stack.length; i++) that.stack[i].isShown && that.stack[i].layout();
                    }, 10);
                });
            }
        },
        ...
        appendModal: function(modal) {
            ...
            var that = this;
            modal.$element.on("show.modalmanager", targetIsSelf(function() {
                var showModal = function() {
                    ...
                    modal.$parent = modal.$element.parent(), modal.$container = that.createContainer(modal), 
                    modal.$element.appendTo(modal.$container), that.backdrop(modal, function() {
                        modal.$element.show(), transition && modal.$element[0].offsetWidth, modal.layout(), 
                        modal.$element.addClass("in").attr("aria-hidden", !1);
                        var complete = function() {
                            that.setFocus(), modal.$element.triggerHandler("shown");
                        };
            ...
            })), modal.$element.on("hidden.modalmanager", targetIsSelf(function() {
                that.backdrop(modal), modal.$backdrop ? $.support.transition && modal.$element.hasClass("fade") ? modal.$backdrop.one($.support.transition.end, function() {
                    that.destroyModal(modal);
                }) : that.destroyModal(modal) : that.destroyModal(modal);
            })), modal.$element.on("destroy.modalmanager", targetIsSelf(function() {
        ...
        destroyModal: function(modal) {
            ...
            this.$element.toggleClass("modal-open", hasOpenModal), hasOpenModal || this.$element.removeClass("page-overflow"), 
            this.removeContainer(modal), this.setFocus();
        },
        hasOpenModal: function() {
            for (var i = 0; i < this.stack.length; i++) if (this.stack[i].isShown) return !0;
            return !1;
        },
        setFocus: function() {
            for (var topModal, i = 0; i < this.stack.length; i++) this.stack[i].isShown && (topModal = this.stack[i]);
            topModal && topModal.focus();
        },
        removeModal: function(modal) {
            modal.$element.off(".modalmanager"), modal.$backdrop && this.removeBackdrop.call(modal), 
        ...
        getIndexOfModal: function(modal) {
            for (var i = 0; i < this.stack.length; i++) if (modal === this.stack[i]) return i;
        },
        replace: function(callback) {
            for (var topModal, i = 0; i < this.stack.length; i++) this.stack[i].isShown && (topModal = this.stack[i]);
            topModal ? (this.$backdropHandle = topModal.$backdrop, topModal.$backdrop = null, 
            callback && topModal.$element.one("hidden", targetIsSelf($.proxy(callback, this))), 
            topModal.hide()) : callback && callback();
        },
        removeBackdrop: function(modal) {
            modal.$backdrop.remove(), modal.$backdrop = null;
        },
        createBackdrop: function(animate) {
            var $backdrop;
            return this.$backdropHandle ? ($backdrop = this.$backdropHandle, $backdrop.off(".modalmanager"), 
            this.$backdropHandle = null, this.isLoading && this.removeSpinner()) : $backdrop = $('<div class="modal-backdrop ' + animate + '" />').appendTo(this.$element), 
            $backdrop;
        },
        removeContainer: function(modal) {
            modal.$container.remove(), modal.$container = null;
        },
        createContainer: function(modal) {
            var $container;
            return $container = $('<div class="modal-scrollable">').css("z-index", getzIndex("modal", modal ? this.getIndexOfModal(modal) : this.stack.length)).appendTo(this.$element), 
            modal && "static" != modal.options.backdrop ? $container.on("click.modal", targetIsSelf(function() {
                modal.hide();
            })) : modal && $container.on("click.modal", targetIsSelf(function() {
                modal.attention();
            })), $container;
        },
        backdrop: function(modal, callback) {
            ...
            if (modal.isShown && showBackdrop) {
                ...
                modal.$backdrop = this.createBackdrop(animate), modal.$backdrop.css("z-index", getzIndex("backdrop", this.getIndexOfModal(modal))), 
                doAnimate && modal.$backdrop[0].offsetWidth, modal.$backdrop.addClass("in"), this.backdropCount += 1, 
                doAnimate ? modal.$backdrop.one($.support.transition.end, callback) : callback();
            } else if (!modal.isShown && modal.$backdrop) {
                modal.$backdrop.removeClass("in"), this.backdropCount -= 1;
                var that = this;
        ...
        loading: function(callback) {
            if (callback = callback || function() {}, this.$element.toggleClass("modal-open", !this.isLoading || this.hasOpenModal()).toggleClass("page-overflow", $(window).height() < this.$element.height()), 
            this.isLoading) if (this.isLoading && this.$backdropHandle) {
            ...
            } else callback && callback(this.isLoading); else {
                this.$backdropHandle = this.createBackdrop("fade"), this.$backdropHandle[0].offsetWidth, 
                ...
                this.isLoading = !0, $.support.transition ? this.$backdropHandle.one($.support.transition.end, callback) : callback();
            }
        }
    };
    var getzIndex = function() {
        ...
        return function(type, pos) {
            if ("undefined" == typeof zIndexFactor) {
                var $baseModal = $('<div class="modal hide" />').appendTo("body"), $baseBackdrop = $('<div class="modal-backdrop hide" />').appendTo("body");
                baseIndex.modal = +$baseModal.css("z-index"), baseIndex.backdrop = +$baseBackdrop.css("z-index"), 
                zIndexFactor = baseIndex.modal - baseIndex.backdrop, $baseModal.remove(), $baseBackdrop.remove(), 
    ...
    $.fn.modalmanager = function(option, args) {
        return this.each(function() {
            ...
            data || $this.data("modalmanager", data = new ModalManager(this, option)), "string" == typeof option && data[option].apply(data, [].concat(args));
        });
    ...
    Modal.prototype = {
        ...
        init: function(element, options) {
            ...
            var manager = "function" == typeof this.options.manager ? this.options.manager.call(this) : this.options.manager;
        ...
        show: function() {
            var e = $.Event("show");
            this.isShown || (this.$element.triggerHandler(e), e.isDefaultPrevented() || (this.escape(), 
            this.tab(), this.options.loading && this.loading()));
        },
        hide: function(e) {
            return e && e.preventDefault(), e = $.Event("hide"), this.$element.triggerHandler(e), 
            !this.isShown || e.isDefaultPrevented() ? this.isShown = !1 : (this.isShown = !1, 
            this.escape(), this.tab(), this.isLoading && this.loading(), $(document).off("focusin.modal"), 
        ...
        layout: function() {
            ...
            if (this.options.width) {
                ...
                this.$element.css("margin-left", function() {
                    return /%/gi.test(that.options.width) ? -(parseInt(that.options.width) / 2) + "%" : -($(this).width() / 2) + "px";
                });
            } else this.$element.css("width", ""), this.$element.css("margin-left", "");
            this.$element.find(".modal-body").css("overflow", "").css(prop, "");
            var modalOverflow = $(window).height() - 10 < this.$element.height();
            value && this.$element.find(".modal-body").css("overflow", "auto").css(prop, value), 
            modalOverflow || this.options.modalOverflow ? this.$element.css("margin-top", 0).addClass("modal-overflow") : this.$element.css("margin-top", 0 - this.$element.height() / 2).removeClass("modal-overflow");
        },
        tab: function() {
            var that = this;
            this.isShown && this.options.consumeTab ? this.$element.on("keydown.tabindex.modal", "[data-tabindex]", function(e) {
                if (e.keyCode && 9 == e.keyCode) {
                    var $next = $(this), $rollover = $(this);
                    that.$element.find("[data-tabindex]:enabled:not([readonly])").each(function(e) {
                        $next = e.shiftKey ? $next.data("tabindex") > $(this).data("tabindex") ? $next = $(this) : $rollover = $(this) : $next.data("tabindex") < $(this).data("tabindex") ? $next = $(this) : $rollover = $(this);
                    }), $next[0] !== $(this)[0] ? $next.focus() : $rollover.focus(), e.preventDefault();
                }
        ...
        hideModal: function() {
            ...
            var prop = this.options.height ? "height" : "max-height", value = this.options.height || this.options.maxHeight;
            value && this.$element.find(".modal-body").css("overflow", "").css(prop, "");
        },
        removeLoading: function() {
        ...
        loading: function(callback) {
            ...
            if (this.isLoading) if (this.isLoading && this.$loading) {
                ...
                doAnimate && this.$loading[0].offsetWidth, this.$loading.addClass("in"), this.isLoading = !0, 
                doAnimate ? this.$loading.one($.support.transition.end, callback) : callback();
            }
        },
        focus: function() {
            var $focusElem = this.$element.find(this.options.focusOn);
            $focusElem = $focusElem.length ? $focusElem : this.$element, $focusElem.focus();
        },
        attention: function() {
        ...
        teardown: function() {
            return this.$parent.length ? (this.$parent !== this.$element.parent() && this.$element.appendTo(this.$parent), 
            this.$element.off(".modal"), this.$element.removeData("modal"), this.$element.removeClass("in").attr("aria-hidden", !0), 
            void 0) : (this.$element.remove(), this.$element = null, void 0);
        }
    }, $.fn.modal = function(option, args) {
        return this.each(function() {
            var $this = $(this), data = $this.data("modal"), options = $.extend({}, $.fn.modal.defaults, $this.data(), "object" == typeof option && option);
            data || $this.data("modal", data = new Modal(this, options)), "string" == typeof option ? data[option].apply(data, [].concat(args)) : options.show && data.show();
        });
    }, $.fn.modal.defaults = {
        keyboard: !0,
        backdrop: !0,
...
}(window.jQuery), function(window, angular, undefined) {
    angular.module("ngAnimate", [ "ng" ]).factory("$$animateReflow", [ "$window", "$timeout", "$document", function($window, $timeout, $document) {
        var bod = $document[0].body, requestAnimationFrame = $window.requestAnimationFrame || $window.webkitRequestAnimationFrame || function(fn) {
        ...
        var timer, queue = [];
        return function(fn) {
            $timeout.cancel(timer), queue.push(fn), timer = $timeout(function() {
        ...
        $provide.decorator("$animate", [ "$delegate", "$injector", "$sniffer", "$rootElement", "$$asyncQueueBuffer", "$rootScope", "$document", function($delegate, $injector, $sniffer, $rootElement, $$asyncQueueBuffer, $rootScope) {
            ...
            function performAnimation(animationEvent, className, element, parentElement, afterElement, domOperation, doneCallback) {
                function onBeforeAnimationsComplete(cancelled) {
                    ...
                    if (cancelled = cancelled || !data || !data.active[className] || isClassBased && data.active[className].event != animationEvent, 
                    fireDOMOperation(), cancelled === !0) return closeAnimation(), void 0;
                    var currentAnimation = data.active[className];
                    currentAnimation.done = closeAnimation, invokeRegisteredAnimationFns(animations, "after", closeAnimation);
                }
                function invokeRegisteredAnimationFns(animations, phase, allAnimationFnsComplete) {
                    ...
                    forEach(animations, function(animation, index) {
                        ...
                        return "before" != phase || "enter" != animationEvent && "move" != animationEvent ? (animation[phase] ? animation[endFnName] = setClassOperation ? animation[phase](element, classNameAdd, classNameRemove, animationPhaseCompleted) : isClassBased ? animation[phase](element, className, animationPhaseCompleted) : animation[phase](element, animationPhaseCompleted) : animationPhaseCompleted(), 
                        void 0) : (animationPhaseCompleted(), void 0);
                    });
                }
                function fireDOMCallback(animationPhase) {
                    var eventName = "$animate:" + animationPhase;
                    elementEvents && elementEvents[eventName] && elementEvents[eventName].length > 0 && $$asyncQueueBuffer(function() {
                ...
                function fireDoneCallbackAsync() {
                    fireDOMCallback("close"), doneCallback && $$asyncQueueBuffer(function() {
                        doneCallback();
                    });
                }
                ...
                function closeAnimation() {
                    if (!closeAnimation.hasBeenRun) {
                        ...
                        var data = element.data(NG_ANIMATE_STATE);
                        data && (isClassBased ? cleanup(element, className) : ($$asyncQueueBuffer(function() {
                            var data = element.data(NG_ANIMATE_STATE) || {};
                ...
                var classNameAdd, classNameRemove, setClassOperation = "setClass" == animationEvent;
                setClassOperation && (classNameAdd = className[0], classNameRemove = className[1], 
                ...
                var matches = lookup(animationLookup), isClassBased = "addClass" == animationEvent || "removeClass" == animationEvent || setClassOperation, ngAnimateState = element.data(NG_ANIMATE_STATE) || {}, runningAnimations = ngAnimateState.active || {}, totalActiveAnimations = ngAnimateState.totalActive || 0, lastAnimation = ngAnimateState.last;
                ...
                var animations = [], allowAnimations = isClassBased ? !ngAnimateState.disabled && (!lastAnimation || lastAnimation.classBased) : !0;
                ...
                if (totalActiveAnimations > 0) {
                    ...
                    animationsToCancel.length > 0 && angular.forEach(animationsToCancel, function(operation) {
                        (operation.done || noop)(!0), cancelAnimations(operation.animations);
                    });
                }
                if (!isClassBased || setClassOperation || skipAnimation || (skipAnimation = "addClass" == animationEvent == element.hasClass(className)), 
                skipAnimation) return fireBeforeCallbackAsync(), fireAfterCallbackAsync(), fireDoneCallbackAsync(), 
                void 0;
                ...
                }, totalActiveAnimations++, runningAnimations[className] = lastAnimation, element.data(NG_ANIMATE_STATE, {
                    last: lastAnimation,
            ...
            function cancelChildAnimations(element) {
                ...
                forEach(node.querySelectorAll("." + NG_ANIMATE_CLASS_NAME), function(element) {
                    ...
                    var data = element.data(NG_ANIMATE_STATE);
                    data && data.active && angular.forEach(data.active, function(operation) {
                        (operation.done || noop)(!0), cancelAnimations(operation.animations);
                    });
                });
            }
            ...
            return {
                ...
                leave: function(element, doneCallback) {
                    cancelChildAnimations(element), this.enabled(!1, element), $rootScope.$$postDigest(function() {
                        element = stripCommentsFromElement(element), performAnimation("leave", "ng-leave", element, null, null, function() {
            ...
            function animationCloseHandler(element, totalTime) {
                ...
                if (!(closingTimestamp >= futureTimestamp)) {
                    $timeout.cancel(closingTimer);
                    var node = extractElementNode(element);
                    element = angular.element(node), animationElementQueue.push(element), closingTimestamp = futureTimestamp, 
            ...
            function animateSetup(animationEvent, element, className, calculationDecorator) {
                ...
                animationDuration > 0 && blockKeyframeAnimations(element), !0;
            }
    ...
    angular.module("mgcrea.ngStrap.affix", [ "mgcrea.ngStrap.helpers.dimensions" ]).provider("$affix", function() {
        ...
        this.$get = [ "$window", "dimensions", function($window, dimensions) {
            function AffixFactory(element, config) {
                ...
                var $affix = {}, options = angular.extend({}, defaults, config), targetEl = options.target, reset = "affix affix-top affix-bottom", initialAffixTop = 0, initialOffsetTop = 0, affixed = null, unpin = null, parent = element.parent();
                ...
                var offsetTop = 0;
                options.offsetTop && ("auto" === options.offsetTop && (options.offsetTop = "+0"), 
                ...
                var offsetBottom = 0;
                return options.offsetBottom && (offsetBottom = options.offsetParent && options.offsetBottom.match(/^[-+]\d+$/) ? getScrollHeight() - (dimensions.offset(parent[0]).top + dimensions.height(parent[0])) + 1 * options.offsetBottom + 1 : 1 * options.offsetBottom), 
                $affix.init = function() {
                    initialOffsetTop = dimensions.offset(element[0]).top + initialAffixTop, targetEl.on("scroll", this.checkPosition), 
                    targetEl.on("click", this.checkPositionWithEventLoop), this.checkPosition(), this.checkPositionWithEventLoop();
                }, $affix.destroy = function() {
                    targetEl.off("scroll", this.checkPosition), targetEl.off("click", this.checkPositionWithEventLoop);
        ...
        this.$get = [ "$modal", "$timeout", function($modal, $timeout) {
            function AlertFactory(config) {
                ...
                $alert = $modal(options), options.type && ($alert.$scope.type = options.type);
                var show = $alert.show;
    ...
    }).directive("bsCheckbox", [ "$button", "$$animateReflow", function($button, $$animateReflow) {
    ...
    }).directive("bsRadio", [ "$button", "$$animateReflow", function($button, $$animateReflow) {
        var defaults = $button.defaults, constantValueRegExp = /^(true|false|\d+)$/;
        return {
            restrict: "A",
            require: "ngModel",
        ...
        this.$get = [ "$window", "$document", "$rootScope", "$sce", "$locale", "dateFilter", "datepickerViews", "$tooltip", function($window, $document, $rootScope, $sce, $locale, dateFilter, datepickerViews, $tooltip) {
            function DatepickerFactory(element, controller, config) {
                ...
                $datepicker.init = function() {
                    return isNative && options.useNative ? (element.prop("type", "date"), element.css("-webkit-appearance", "textfield"), 
                    void 0) : (isTouch && (element.prop("type", "text"), element.attr("readonly", "true"), 
                    element.on("click", focusElement)), _init(), void 0);
                };
        ...
        {
            var isNative = ($datepicker.defaults, /(ip(a|o)d|iphone|android)/gi.test($window.navigator.userAgent));
            $window.requestAnimationFrame || $window.setTimeout;
        }
        return {
            restrict: "EAC",
            ...
            link: function(scope, element, attr, controller) {
                ...
                options = datepicker.$options, angular.forEach([ "minDate", "maxDate" ], function(key) {
                    angular.isDefined(attr[key]) && attr.$observe(key, function(newValue) {
                        ...
                        } else datepicker.$options[key] = angular.isString(newValue) && newValue.match(/^".+"$/) ? +new Date(newValue.substr(1, newValue.length - 2)) : +new Date(newValue);
                ...
                controller.$parsers.unshift(function(viewValue) {
                    if (!viewValue) return controller.$setValidity("date", !0), void 0;
                    var parsedDate = dateParser.parse(viewValue, controller.$dateValue);
                    if (!parsedDate || isNaN(parsedDate.getTime())) controller.$setValidity("date", !1); else {
                        var isValid = parsedDate.getTime() >= options.minDate && parsedDate.getTime() <= options.maxDate;
                        controller.$setValidity("date", isValid), isValid && (controller.$dateValue = parsedDate);
                    }
                    return "string" === options.dateType ? dateFilter(viewValue, options.dateFormat) : "number" === options.dateType ? controller.$dateValue.getTime() : "iso" === options.dateType ? controller.$dateValue.toISOString() : controller.$dateValue;
                }), controller.$formatters.push(function(modelValue) {
                    var date = angular.isDate(modelValue) ? modelValue : new Date(modelValue);
                    return controller.$dateValue = date, controller.$dateValue;
                }), controller.$render = function() {
        ...
        this.$get = [ "$locale", "$sce", "dateFilter", function($locale, $sce, dateFilter) {
            return function(picker) {
                var scope = picker.$scope, options = picker.$options, weekDaysMin = $locale.DATETIME_FORMATS.SHORTDAY, weekDaysLabels = weekDaysMin.slice(options.startWeek).concat(weekDaysMin.slice(0, options.startWeek)), weekDaysLabelsHtml = $sce.trustAsHtml('<th class="dow text-center">' + weekDaysLabels.join('</th><th class="dow text-center">') + "</th>"), startDate = picker.$date || new Date(), viewDate = {
                    ...
                    build: function() {
                        for (var day, firstDayOfMonth = new Date(viewDate.year, viewDate.month, 1), firstDate = new Date(+firstDayOfMonth - 864e5 * (firstDayOfMonth.getDay() + options.startWeek)), days = [], i = 0; 42 > i; i++) day = new Date(firstDate.getFullYear(), firstDate.getMonth(), firstDate.getDate() + i), 
                        days.push({
                            date: day,
                        ...
                        scope.title = dateFilter(firstDayOfMonth, "MMMM yyyy"), scope.labels = weekDaysLabelsHtml, 
                    ...
                    onKeyDown: function(evt) {
                        ...
                        37 === evt.keyCode ? picker.select(new Date(actualTime - 864e5), !0) : 38 === evt.keyCode ? picker.select(new Date(actualTime - 6048e5), !0) : 39 === evt.keyCode ? picker.select(new Date(actualTime + 864e5), !0) : 40 === evt.keyCode && picker.select(new Date(actualTime + 6048e5), !0);
                    }
                }, {
                    name: "month",
                    format: "MMM",
                    ...
                    build: function() {
                        ...
                        scope.title = dateFilter(month, "yyyy"), scope.labels = !1, scope.rows = split(months, this.split), 
                        this.built = !0;
                    },
                    isSelected: function(date) {
                    ...
                    onKeyDown: function(evt) {
                        ...
                        37 === evt.keyCode ? picker.select(picker.$date.setMonth(actualMonth - 1), !0) : 38 === evt.keyCode ? picker.select(picker.$date.setMonth(actualMonth - 4), !0) : 39 === evt.keyCode ? picker.select(picker.$date.setMonth(actualMonth + 1), !0) : 40 === evt.keyCode && picker.select(picker.$date.setMonth(actualMonth + 4), !0);
                    }
                }, {
                    name: "year",
                    format: "yyyy",
                    ...
                    build: function() {
                        ...
                        scope.title = years[0].label + "-" + years[years.length - 1].label, scope.labels = !1, 
                    ...
                    onKeyDown: function(evt) {
                        ...
                        37 === evt.keyCode ? picker.select(picker.$date.setYear(actualYear - 1), !0) : 38 === evt.keyCode ? picker.select(picker.$date.setYear(actualYear - 4), !0) : 39 === evt.keyCode ? picker.select(picker.$date.setYear(actualYear + 1), !0) : 40 === evt.keyCode && picker.select(picker.$date.setYear(actualYear + 4), !0);
                    }
                } ]);
                return {
        ...
        this.$get = [ "$window", "$tooltip", function($window, $tooltip) {
        ...
        this.$get = [ "$locale", function($locale) {
            var DateParserFactory = function(config) {
                ...
                return $dateParser.init = function() {
                    ...
                    if (!matches) return !1;
                    for (var date = baseDate || new Date(0), i = 0; i < matches.length - 1; i++) setMap[i] && setMap[i].call(date, matches[i + 1]);
                    return date;
            ...
            function ParseOptionsFactory(attr, config) {
                function parseValues(values) {
                    return values.map(function(match, index) {
                        ...
                        return locals[valueName] = match, label = displayFn(locals), value = valueFn(locals) || index, 
                        {
                            label: label,
                            value: value
                        };
                ...
                return $parseOptions.init = function() {
                    ...
                    return $q.when(valuesFn(scope, controller)).then(function(values) {
                        return $parseOptions.$values = values ? parseValues(values) : {}, $parseOptions.$values;
                    });
        ...
        this.$get = [ "$window", "$rootScope", "$compile", "$q", "$templateCache", "$http", "$animate", "$timeout", "dimensions", function($window, $rootScope, $compile, $q, $templateCache, $http, $animate) {
            function ModalFactory(config) {
                function hideOnBackdropClick(evt) {
                    evt.target === evt.currentTarget && ("static" === options.backdrop ? $modal.focus() : $modal.hide());
                }
                var $modal = {}, options = angular.extend({}, defaults, config);
                $modal.$promise = fetchTemplate(options.template);
                var scope = $modal.$scope = options.scope && options.scope.$new() || $rootScope.$new();
                options.element || options.container || (options.container = "body"), options.scope || forEach([ "title", "content" ], function(key) {
                    options[key] && (scope[key] = options[key]);
                }), scope.$hide = function() {
                ...
                return $modal.$promise.then(function(template) {
                    ...
                    $animate.enter(modalElement, parent, after, function() {}), scope.$isShown = !0, 
                    scope.$$phase || scope.$digest();
                    var el = modalElement[0];
                    requestAnimationFrame(function() {
                        el.focus();
                    }), bodyElement.addClass(options.prefixClass + "-open"), options.backdrop && (modalElement.on("click", hideOnBackdropClick), 
                    backdropElement.on("click", hideOnBackdropClick)), options.keyboard && modalElement.on("keyup", $modal.$onKeyUp);
                }, $modal.hide = function() {
                    $animate.leave(modalElement, function() {
                        bodyElement.removeClass(options.prefixClass + "-open");
                    }), options.backdrop && $animate.leave(backdropElement, function() {}), scope.$isShown = !1, 
                    scope.$$phase || scope.$digest(), options.backdrop && (modalElement.off("click", hideOnBackdropClick), 
            ...
            link: function(scope, element, attr) {
                ...
                angular.forEach(Object.keys(defaults), function(key) {
                    ...
                    angular.forEach(liElements, function(li) {
                        var liElement = angular.element(li), pattern = liElement.attr(options.routeAttr), regexp = new RegExp("^" + pattern.replace("/", "\\/") + "$", [ "i" ]);
            ...
            link: function(scope, element, attr) {
                ...
                angular.forEach([ "template", "contentTemplate", "placement", "container", "delay", "trigger", "keyboard", "html", "animation" ], function(key) {
                    ...
                    attr[key] && attr.$observe(key, function(newValue, oldValue) {
                        scope[key] = $sce.getTrustedHtml(newValue), angular.isDefined(oldValue) && requestAnimationFrame(function() {
        ...
        this.$get = [ "$window", "$document", "$rootScope", "dimensions", "debounce", "throttle", function($window, $document, $rootScope, dimensions, debounce, throttle) {
            ...
            function ScrollSpyFactory(config) {
                ...
                var activeTarget, debouncedCheckPosition, throttledCheckPosition, debouncedCheckOffsets, viewportHeight, scrollTop, $scrollspy = {}, trackedElements = $scrollspy.$trackedElements = [], sortedElements = [];
                return $scrollspy.init = function() {
                    ...
                    debouncedCheckOffsets = debounce(this.checkOffsets, options.debounce), $rootScope.$on("$viewContentLoaded", debouncedCheckOffsets), 
                    $rootScope.$on("$includeContentLoaded", debouncedCheckOffsets), debouncedCheckOffsets(), 
                    scrollId && (spies[scrollId] = $scrollspy);
                    ...
                    $rootScope.$off("$viewContentLoaded", debouncedCheckOffsets), $rootScope.$off("$includeContentLoaded", debouncedCheckOffsets));
        ...
        this.$get = [ "$window", "$document", "$rootScope", "$tooltip", function($window, $document, $rootScope, $tooltip) {
            function SelectFactory(element, controller, config) {
                ...
                var parentScope = config.scope, scope = $select.$scope;
                ...
                scope.$activate = function(index) {
                    ...
                    })) : controller.$setViewValue(value), controller.$render(), parentScope && parentScope.$digest(), 
                    options.multiple || ("focus" === options.trigger ? element[0].blur() : $select.$isShown && $select.hide()), 
                    ...
                    evt.preventDefault(), evt.stopPropagation(), $select.$isShown ? element[0].blur() : element[0].focus();
                    ...
                    if (/(38|40|13)/.test(evt.keyCode)) {
                        if (evt.preventDefault(), evt.stopPropagation(), 13 === evt.keyCode) return $select.select(scope.$activeIndex);
                ...
                $select.init = function() {
                    _init(), element.on(isTouch ? "touchstart" : "mousedown", $select.$onElementMouseDown);
                };
                var _destroy = $select.destroy;
                $select.destroy = function() {
                    _destroy(), element.off(isTouch ? "touchstart" : "mousedown", $select.$onElementMouseDown);
                };
                ...
                return $select.hide = function() {
                    ...
                    options.keyboard && element.off("keydown", $select.$onKeyDown), _hide();
                }, $select;
            }
            var isTouch = (angular.element($window.document.body), "createTouch" in $window.document);
        ...
        return {
            ...
            link: function(scope, element, attr, controller) {
                ...
                if (angular.forEach([ "placement", "container", "delay", "trigger", "keyboard", "html", "animation", "template", "placeholder", "multiple" ], function(key) {
                    angular.isDefined(attr[key]) && (options[key] = attr[key]);
                ...
                scope.$watch(watchedOptions, function() {
                    ...
                    }).filter(angular.isDefined).join(", ") : (index = select.$getIndex(controller.$modelValue), 
        ...
        this.$get = [ "$window", "$document", "$rootScope", "$sce", "$locale", "dateFilter", "$tooltip", function($window, $document, $rootScope, $sce, $locale, dateFilter, $tooltip) {
            function timepickerFactory(element, controller, config) {
                ...
                scope.$select = function(date, index) {
                    ...
                    isNaN(controller.$dateValue.getTime()) && (controller.$dateValue = new Date(1970, 0, 1)), 
                    ...
                    controller.$dateValue.setHours(12 > hours ? hours + 12 : hours - 12), controller.$render();
                    ...
                    scope.rows = rows, scope.showAM = !!formats[2], scope.isAM = ($timepicker.$date || hours[midIndex].date).getHours() < 12, 
                    $timepicker.$isBuilt = !0;
                    ...
                    selectedTime < options.minTime || selectedTime > options.maxTime;
                }, $timepicker.$moveIndex = function(value, index) {
                    var targetDate;
                    0 === index ? (targetDate = new Date(1970, 0, 1, viewDate.hour + value * options.length, viewDate.minute), 
                    angular.extend(viewDate, {
                        hour: targetDate.getHours()
                    })) : 1 === index && (targetDate = new Date(1970, 0, 1, viewDate.hour, viewDate.minute + value * options.length * 5), 
                    angular.extend(viewDate, {
                    ...
                    if (/(38|37|39|40|13)/.test(evt.keyCode) && !evt.shiftKey && !evt.altKey) {
                        ...
                        var newDate = new Date($timepicker.$date), hours = newDate.getHours(), hoursLength = dateFilter(newDate, "h").length, minutes = newDate.getMinutes(), minutesLength = dateFilter(newDate, "mm").length, lateralMove = /(37|39)/.test(evt.keyCode), count = 2 + 1 * !!formats[2];
                        if (lateralMove && (37 === evt.keyCode ? selectedIndex = 1 > selectedIndex ? count - 1 : selectedIndex - 1 : 39 === evt.keyCode && (selectedIndex = count - 1 > selectedIndex ? selectedIndex + 1 : 0)), 
                        0 === selectedIndex) {
                            if (lateralMove) return createSelection(0, hoursLength);
                            38 === evt.keyCode ? newDate.setHours(hours - options.hourStep) : 40 === evt.keyCode && newDate.setHours(hours + options.hourStep);
                        } else if (1 === selectedIndex) {
                            if (lateralMove) return createSelection(hoursLength + 1, hoursLength + 1 + minutesLength);
                            38 === evt.keyCode ? newDate.setMinutes(minutes - options.minuteStep) : 40 === evt.keyCode && newDate.setMinutes(minutes + options.minuteStep);
                        ...
                        $timepicker.select(newDate, selectedIndex, !0), parentScope.$digest();
                    }
                };
                var _init = $timepicker.init;
                $timepicker.init = function() {
                    return isNative && options.useNative ? (element.prop("type", "time"), element.css("-webkit-appearance", "textfield"), 
                    void 0) : (isTouch && (element.prop("type", "text"), element.attr("readonly", "true"), 
                    element.on("click", focusElement)), _init(), void 0);
                };
        ...
        {
            var isNative = ($timepicker.defaults, /(ip(a|o)d|iphone|android)/gi.test($window.navigator.userAgent));
            $window.requestAnimationFrame || $window.setTimeout;
        }
        return {
            restrict: "EAC",
            ...
            link: function(scope, element, attr, controller) {
                ...
                angular.forEach([ "placement", "container", "delay", "trigger", "keyboard", "html", "animation", "template", "autoclose", "timeType", "timeFormat", "useNative", "lang" ], function(key) {
                    angular.isDefined(attr[key]) && (options[key] = attr[key]);
                }), isNative && options.useNative && (options.timeFormat = "HH:mm");
                ...
                angular.forEach([ "minTime", "maxTime" ], function(key) {
                    angular.isDefined(attr[key]) && attr.$observe(key, function(newValue) {
                        timepicker.$options[key] = "now" === newValue ? new Date().setFullYear(1970, 0, 1) : angular.isString(newValue) && newValue.match(/^".+"$/) ? +new Date(newValue.substr(1, newValue.length - 2)) : dateParser.parse(newValue), 
                    ...
                    return "string" === options.timeType ? dateFilter(viewValue, options.timeFormat) : "number" === options.timeType ? controller.$dateValue.getTime() : "iso" === options.timeType ? controller.$dateValue.toISOString() : controller.$dateValue;
                }), controller.$formatters.push(function(modelValue) {
                    var date = angular.isDate(modelValue) ? modelValue : new Date(modelValue);
                    return controller.$dateValue = date, controller.$dateValue;
                }), controller.$render = function() {
        ...
        this.$get = [ "$window", "$rootScope", "$compile", "$q", "$templateCache", "$http", "$animate", "$timeout", "dimensions", "$$animateReflow", function($window, $rootScope, $compile, $q, $templateCache, $http, $animate, $timeout, dimensions, $$animateReflow) {
            function TooltipFactory(element, config) {
                function getPosition() {
                    return "body" === options.container ? dimensions.offset(element[0]) : dimensions.position(element[0]);
                }
                ...
                var $tooltip = {}, options = $tooltip.$options = angular.extend({}, defaults, config);
                ...
                options.contentTemplate && ($tooltip.$promise = $tooltip.$promise.then(function(template) {
                    var templateEl = angular.element(template);
                    return fetchTemplate(options.contentTemplate).then(function(contentTemplate) {
                        return findElement('[ng-bind="content"]', templateEl[0]).removeAttr("ng-bind").html(contentTemplate), 
                        templateEl[0].outerHTML;
                    });
                }));
                ...
                return $tooltip.$promise.then(function(template) {
                    ...
                    angular.forEach(triggers, function(trigger) {
                        ...
                        element.on("hover" === trigger ? "mouseleave" : "blur", $tooltip.leave));
                    }), options.show && scope.$$postDigest(function() {
                    ...
                    for (var triggers = options.trigger.split(" "), i = triggers.length; i--; ) {
                        ...
                        element.off("hover" === trigger ? "mouseleave" : "blur", $tooltip.leave));
                    }
                    tipElement && (tipElement.remove(), tipElement = null), scope.$destroy();
                }, $tooltip.enter = function() {
                    return clearTimeout(timeout), hoverState = "in", options.delay && options.delay.show ? (timeout = setTimeout(function() {
                        "in" === hoverState && $tooltip.show();
                    }, options.delay.show), void 0) : $tooltip.show();
                    ...
                    $tooltip.$isShown = !0, scope.$$phase || scope.$digest(), $$animateReflow($tooltip.$applyPlacement), 
                    ...
                    return clearTimeout(timeout), hoverState = "out", options.delay && options.delay.hide ? (timeout = setTimeout(function() {
                        "out" === hoverState && $tooltip.hide();
                    }, options.delay.hide), void 0) : $tooltip.hide();
                }, $tooltip.hide = function(blur) {
                    return $tooltip.$isShown ? ($animate.leave(tipElement, function() {
                        tipElement = null;
                    }), scope.$$phase || scope.$digest(), $tooltip.$isShown = !1, options.keyboard && tipElement.off("keyup", $tooltip.$onKeyUp), 
                    blur && "focus" === options.trigger ? element[0].blur() : void 0) : void 0;
            ...
            var trim = String.prototype.trim, htmlReplaceRegExp = ("createTouch" in $window.document, 
            /ng-bind="/gi);
            return TooltipFactory;
        } ];
    }).directive("bsTooltip", [ "$window", "$location", "$sce", "$tooltip", "$$animateReflow", function($window, $location, $sce, $tooltip, $$animateReflow) {
        return {
            restrict: "EAC",
            scope: !0,
            link: function(scope, element, attr) {
                ...
                angular.forEach([ "template", "contentTemplate", "placement", "container", "delay", "trigger", "keyboard", "html", "animation", "type" ], function(key) {
                    ...
                    attr[key] && attr.$observe(key, function(newValue, oldValue) {
                        scope[key] = $sce.getTrustedHtml(newValue), angular.isDefined(oldValue) && $$animateReflow(function() {
        ...
        this.$get = [ "$window", "$rootScope", "$tooltip", function($window, $rootScope, $tooltip) {
            function TypeaheadFactory(element, config) {
                var $typeahead = {}, options = angular.extend({}, defaults, config), controller = options.controller;
                $typeahead = $tooltip(element, options);
                var parentScope = config.scope, scope = $typeahead.$scope;
                scope.$matches = [], scope.$activeIndex = 0, scope.$activate = function(index) {
                    scope.$$postDigest(function() {
                    ...
                    var value = scope.$matches[index].value;
                    controller && (controller.$setViewValue(value), controller.$render(), parentScope && parentScope.$digest()), 
                    "focus" === options.trigger ? element[0].blur() : $typeahead.$isShown && $typeahead.hide(), 
                    scope.$activeIndex = 0, scope.$emit("$typeahead.select", value, index);
                    ...
                    if (/(38|40|13)/.test(evt.keyCode)) {
                        if (evt.preventDefault(), evt.stopPropagation(), 13 === evt.keyCode) return $typeahead.select(scope.$activeIndex);
                        38 === evt.keyCode && scope.$activeIndex > 0 ? scope.$activeIndex-- : 40 === evt.keyCode && scope.$activeIndex < scope.$matches.length - 1 ? scope.$activeIndex++ : angular.isUndefined(scope.$activeIndex) && (scope.$activeIndex = 0), 
                        scope.$digest();
                    }
                };
        ...
        return {
            ...
            link: function(scope, element, attr, controller) {
                ...
                var limit = options.limit || defaults.limit, parsedOptions = $parseOptions(attr.ngOptions + " | filter:$viewValue | limitTo:" + limit), typeahead = $typeahead(element, options);
                scope.$watch(attr.ngModel, function() {
                    parsedOptions.valuesFn(scope, controller).then(function(values) {
                        values.length > limit && (values = values.slice(0, limit)), typeahead.update(values);
                    });
                }), scope.$on("$destroy", function() {
    ...
    angular.module("mgcrea.ngStrap.alert").run([ "$templateCache", function($templateCache) {
        ...
        $templateCache.put("dropdown/dropdown.tpl.html", '<ul tabindex="-1" class="dropdown-menu" role="menu"><li role="presentation" ng-class="{divider: item.divider}" ng-repeat="item in content"><a role="menuitem" tabindex="-1" href="{{item.href}}" ng-if="!item.divider" ng-click="$eval(item.click);$hide()" ng-bind="item.text"></a></li></ul>');
    ...
    var cc = {
        ...
        cardType: function(cardNumber) {
            ...
            p["4"] = "VISA", p["6"] = "Discover Card", cardNumber) for (var k in p) if (0 === cardNumber.indexOf(k)) return p[k];
            return null;
        },
        ...
        validate: function(cardData) {
            cardData.card_number && (cardData.card_number = cardData.card_number.toString().trim());
            var cardNumber = cardData.card_number, securityCode = cardData.security_code, expiryMonth = cardData.expiration_month, expiryYear = cardData.expiration_year, errors = {};
            return cc.isCardNumberValid(cardNumber) || (errors.card_number = '"' + cardNumber + '" is not a valid credit card number'), 
            "undefined" == typeof securityCode || null === securityCode || cc.isSecurityCodeValid(cardNumber, securityCode) || (errors.security_code = '"' + securityCode + '" is not a valid credit card security code'), 
            cc.isExpiryValid(expiryMonth, expiryYear) || (errors.expiry = '"' + expiryMonth + "-" + expiryYear + '" is not a valid credit card expiry date', 
        ...
        create: function(data, callback) {
            ...
            if (!_marketplace_uri) return noDataError(callback, "You need to call balanced.init first"), 
            void 0;
            var requiredKeys = [ "card_number", "expiration_month", "expiration_year" ], errors = validate(data, requiredKeys, cc.validate), ec = 0;
            for (var p in errors) {
                ec++;
                break;
            }
            if (ec > 0) callback({
                ...
                var uri = _marketplace_uri + "/cards", payload = preparePayload(data);
                sendWhenReady(uri, payload, callback);
            }
        }
    }, em = {
        validate: function(emailAddress) {
            var match = emailAddress && emailAddress.match(/[a-z0-9!#$%&'*+\/=?\^_`{|}~\-]+(?:\.[a-z0-9!#$%&'*+\/=?\^_`{|}~\-]+)*@(?:[a-z0-9](?:[a-z0-9\-]*[a-z0-9])?\.)+[a-z0-9](?:[a-z0-9\-]*[a-z0-9])?/i);
        ...
        validate: function(accountData) {
            ...
            return ba.validateRoutingNumber(bankCode) || (errors[noun] = '"' + bankCode + '" is not a valid ' + noun.replace("_", " ")), 
            "type" in accountData && !ba.validateType(accountData.type) && (errors.type = '"' + accountData.type + '" must be one of: "' + ba.types.join('", "') + '"'), 
            errors;
        },
        validateRoutingNumber: function(routingNumber) {
            if (!routingNumber) return !1;
            if (routingNumber = routingNumber.match(/\d+/g), !routingNumber) return !1;
        ...
        create: function(data, callback) {
            ...
            data && "routing_number" in data && (requiredKeys = [ "name", "account_number", "routing_number" ]);
            var errors = validate(data, requiredKeys, ba.validate), ec = 0;
            for (var p in errors) {
                ec++;
                break;
            }
            if (ec > 0) callback({
                ...
                var uri = _marketplace_uri + "/bank_accounts", payload = preparePayload(data);
                sendWhenReady(uri, payload, callback);
            }
        }
    };
    ctx.balanced = {
        init: function(marketplace_uri, params) {
            params = params || {}, "server" in params && (server = params.server, proxy = server + "/proxy"), 
            "debug" in params && (debug = params.debug, proxy = server + "/devproxy");
            try {
    ...
    var _marketplace_uri, server = "https://js.balancedpayments.com", proxy = server + "/proxy", debug = 0, MARKETPLACE_URI_REGEX = "/v1/marketplaces/(\\w|-)+", ROUTING_NUMBER_URI = "/v1/bank_accounts/routing_numbers/", validate = function(details, requiredKeys, validationMethod) {
        var errors = {};
        validateData(requiredKeys, details, errors);
        var additionalErrors = validationMethod(details);
        for (var k in additionalErrors) errors[k] = additionalErrors[k];
        return errors;
        ...
        el.setAttribute("type", "text/javascript"), el.setAttribute("src", scriptUri), callback && (el.onload = callback, 
        el.onreadystatechange = function() {
            "complete" == this.readyState && callback();
        }), document.getElementsByTagName("head")[0].appendChild(el);
    };
    ("undefined" == typeof JSON || "function" != typeof JSON.stringify || "function" != typeof JSON.parse) && loadScript(server + "/v1/json2.min.js");
    var windowProxy, frame = null, frameName = null, messageQueue = [], callbackQueue = [], timeoutQueue = [], callCount = 0, callbackInitialized = !1, capabilities = {
        system_timezone: -new Date().getTimezoneOffset() / 60,
        user_agent: navigator.userAgent,
        language: navigator.language
    }, initIFrame = function() {
        frame = null;
        var body = document.getElementsByTagName("body")[0], iframe = document.createElement("iframe"), frameName = "balancedFrame" + new Date().getTime(), src = proxy + "#" + encodeURIComponent(window.location.href), attributes = {
            src: src,
            name: frameName,
            id: frameName,
            frameborder: 0,
        ...
        var onLoad = function() {
            frame = window.frames[frameName], processMessages();
        };
        iframe.attachEvent ? iframe.attachEvent("onload", onLoad) : iframe.onload = onLoad, 
        body.appendChild(iframe);
    }, processMessages = function() {
        function processMessage(data) {
            var message = data.message, messageId = message.id;
            callbackQueue[messageId] = data.callback, XD.postMessage(message, proxy, proxy, frame), 
            timeoutQueue[messageId] = window.setTimeout(function() {
                ...
                }), delete callbackQueue[messageId], delete timeoutQueue[messageId];
            }, 6e4);
        }
        var messageQueueLength = messageQueue.length;
        if (frame && messageQueueLength) {
            for (var counter = 0; messageQueueLength > counter; ++counter) {
        ...
        var messageId = response.id, data = response.response;
        try {
            data = JSON.parse(data);
        } catch (e) {}
        data.status = parseInt(data.status, 10), callbackQueue[messageId](data), window.clearTimeout(timeoutQueue[messageId]), 
        delete callbackQueue[messageId], delete timeoutQueue[messageId];
    }, validateData = function(requiredKeys, data, errors) {
        for (var i = 0; i < requiredKeys.length; i++) {
            var key = requiredKeys[i];
            data && key in data && data[key] || (errors[key] = "Missing field");
        }
    }, preparePayload = function(unencryptedDict) {
        for (var k in capabilities) k in unencryptedDict || (unencryptedDict[k] = capabilities[k]);
        return JSON.stringify(unencryptedDict);
    }, sendWhenReady = function(uri, data, callback, method) {
        method = method || "POST", messageQueue.push({
            message: {
                id: callCount++,
                uri: uri,
                params: data,
    ...
    addEvent(window, "keydown", function(e) {
        capabilities.cl || (capabilities.cl = icl(e));
    }), addEvent(window, "paste", function() {
        capabilities.ps = !0;
    });
    var createProxy = function(proxy) {
        windowProxy = proxy ? proxy : XD, messageQueue = [], callbackQueue = {}, timeoutQueue = {};
        var setupIframe = function() {
            initIFrame(), callbackInitialized || (XD.receiveMessage(receiveMessage, server), 
            callbackInitialized = !0);
        };
        frameName && document.getElementById(frameName) || ("undefined" != typeof document && document && document.body ? setupIframe() : "undefined" != typeof window && window && !callbackInitialized && (window.addEventListener ? window.addEventListener("load", setupIframe, !1) : window.attachEvent("onload", setupIframe)));
    };
    ...
    var XD = function() {
        var deserialize = function(data) {
            for (var decoded = {}, keyPairs = data.split("&"), keyPairLength = keyPairs.length, c = null, d = null, i = 0; keyPairLength > i; ++i) {
                d = keyPairs[i].split("="), d[0] = decodeURIComponent(d[0]), d[1] = decodeURIComponent(d[1]);
                for (var f = d[0], c = [], g = -1; -1 !== (g = f.indexOf("[")); ) c.push(f.substr(g, f.indexOf("]") - g + 1)), 
                f = f.substr(f.indexOf("]") + 1);
                if (0 === c.length) decoded[d[0]] = d[1]; else {
                    g = d[0].substr(0, d[0].indexOf("[")), "undefined" == typeof decoded[g] && (decoded[g] = {});
                    for (var f = decoded[g], t = c.length, p = 0; t - 1 > p; ++p) g = c[p].substr(1, c[p].length - 2), 
                    "undefined" == typeof f[g] && (f[g] = {}), f = f[g];
                    c = c[t - 1], g = c.substr(1, c.length - 2), f[g] = d[1];
                }
            }
            return decoded;
            ...
            for (key in data) if (data.hasOwnProperty(key)) {
                var objectKey = prefix ? prefix + "[" + key + "]" : key, value = data[key];
                encoded.push("object" == typeof value ? serialize(value, objectKey) : encodeURIComponent(objectKey) + "=" + encodeURIComponent(value));
            }
            return encoded.join("&");
        };
        return {
            receiveMessage: function(callback, origin) {
                if ("undefined" != typeof window) if (window.postMessage) {
                    var attachedCallback = function(response) {
                        return response.origin.toLowerCase() !== origin.toLowerCase() ? !1 : (callback(deserialize(response.data)), 
                        void 0);
                    };
                    ...
                    setInterval(function() {
                        var newHash = window.location.hash, hashIdentifier = /^#?\d+&/;
                        newHash !== currentHash && hashIdentifier.test(newHash) && (currentHash = newHash, 
            ...
            postMessage: function(messageToSend, callback, iframeSrc, iframe) {
                if ("undefined" != typeof window) if (messageToSend = serialize(messageToSend), 
                "undefined" == typeof window.postMessage) {
                    var url = iframeSrc + "#" + +new Date() + Math.floor(1e3 * Math.random()) + "&" + messageToSend;
                    iframe.location.href = url;
                } else iframe.postMessage(messageToSend, callback);
            }
        };
    }();
    window.balanced = balanced;
}(this), angular.module("wembliApp.controllers", []).controller("TestCtrl", [ "$scope", function() {} ]), 
angular.module("wembliApp.directives", []).directive("toggleSelected", [ function() {
    return {
        restrict: "C",
        link: function(scope, element) {
    ...
    return {
        ...
        compile: function() {
            return function(scope, element, attr) {
                var header = "false" === attr.header ? !1 : !0, ms = element.multiselect({
                ...
                attr.$observe("ngModel", function(m) {
                    ...
                    if ("undefined" != typeof opts) {
                        for (var i = 0; i < opts.length; i++) element.multiselect("widget").find(":checkbox[value='" + opts[i] + "']").attr("checked", "checked"), 
                        $("#" + attr.id + " option[value='" + opts[i] + "']").attr("selected", 1);
                        element.multiselect("refresh");
                    }
                }), scope.$watch(attr.enable, function(newVal) {
                    "undefined" != typeof newVal && (newVal ? ms.multiselect("enable") : ms.multiselect("disable"));
                }), attr.$observe("click", function() {
                    var clickFn = scope.$eval(attr.click);
                    "undefined" != typeof clickFn && element.on("multiselectclick", function(event, ui) {
                        scope.$apply(function() {
        ...
        compile: function() {
            return function(scope, element) {
                element.bind("keydown", function(evt) {
                    "phoneNumber" === document.activeElement.name && 8 != evt.keyCode && 9 != evt.keyCode && (evt.keyCode < 48 || evt.keyCode > 57) && evt.preventDefault();
                });
            };
        }
    };
} ]).directive("formatPostalCode", [ function() {
    return {
        restrict: "C",
        compile: function() {
            return function(scope, element) {
                element.bind("keydown", function(evt) {
                    "postalCode" === document.activeElement.name && 8 != evt.keyCode && 9 != evt.keyCode && 173 != evt.keyCode && (evt.keyCode < 48 || evt.keyCode > 57) && evt.preventDefault();
                });
            };
        }
    };
    ...
    return {
        restrict: "E",
        controller: [ "$scope", "$element", "$attrs", "$transclude", "loadingModal", function($scope, $element, $attrs, $transclude, loadingModal) {
            $scope.loadingModal = {}, $scope.$on("loading-modal-show", function() {
                $scope.loadingModal.title = loadingModal.title, $scope.loadingModal.body = loadingModal.body;
            });
        } ],
        compile: function(element, attr) {
            element.click(function() {
...
} ]).directive("addEvent", [ "wembliRpc", function(wembliRpc) {
    return {
        restrict: "C",
        compile: function(element, attr) {
            var events = {
                click: function() {
                    element.click(function() {
                        wembliRpc.fetch("analytics.addEvent", attr, function() {});
                    });
                }
            };
            events[attr.event]();
    ...
    return {
        ...
        compile: function() {
            return function(scope, element, attr) {
                ...
                "contentId" in attr ? attr.$observe("contentId", function(c) {
                    "undefined" != typeof c && contentId !== c && (contentId = c, element.popover({
                        placement: attr.placement,
                        trigger: attr.trigger,
                        animation: "true" === attr.animation ? !0 : !1,
                        title: attr.title,
                        content: function() {
                            return $("#" + contentId).html();
                        },
                        html: "true" === attr.html ? !0 : !1
                    }));
                }) : attr.$observe("content", function(c) {
                    content !== c && (content = c, element.popover({
                        placement: attr.placement,
                        trigger: attr.trigger,
                        animation: "true" === attr.animation ? !0 : !1,
                        title: attr.title,
    ...
    return function(scope, elm, attrs) {
        elm.bind("keydown", function(evt) {
            var e = evt, keydownFn = scope.$eval(attrs.onKeydown);
            scope.$apply(function() {
    ...
    return {
        ...
        compile: function() {
            return function(scope, element) {
                var focus = function() {
                    $timeout(function() {
                        element.focus(), element.is(":focus") || focus();
                    }, 100);
                };
                element.click(function() {
                    focus();
...
} ]).directive("createBalancedAccount", [ "$rootScope", "pluralize", "wembliRpc", "plan", "loadingModal", function($rootScope, pluralize, wembliRpc) {
    return {
        restrict: "C",
        cache: !1,
        compile: function() {
            return function(scope) {
                scope.createBalancedAccount = function() {
                    console.log("show loading modal"), scope.submitInProgress = !0;
                    var error = !1;
                    if (/^\d{2}[-\/]\d{2}[-\/]\d{4}$/.test(scope.createMerchantAccount.dob) ? scope.createMerchantAccount.errorDob = !1 : (scope.createMerchantAccount.errorDob = !0, 
                    error = !0), /^\d{10}$/.test(scope.createMerchantAccount.phoneNumber) ? scope.createMerchantAccount.errorPhoneNumber = !1 : (scope.createMerchantAccount.errorPhoneNumber = !0, 
                    error = !0), scope.createMerchantAccount.accountHolderError = scope.createMerchantAccount.errorPhoneNumber || scope.createMerchantAccount.errorDob ? !0 : !1, 
                    balanced.bankAccount.validateRoutingNumber(scope.createMerchantAccount.routingNumber) ? scope.createMerchantAccount.errorRoutingNumber = !1 : (scope.createMerchantAccount.errorRoutingNumber = !0, 
                    error = !0), error) return console.log("error creating bank account"), console.log(scope.createMerchantAccount), 
                    scope.submitInProgress = !1, void 0;
                    var dobAry = scope.createMerchantAccount.dob.split(/[-\/]/), dob = dobAry[2] + "-" + dobAry[0] + "-" + dobAry[1], accountInfo = {
                    ...
                    wembliRpc.fetch("customer.createMerchantAccount", accountInfo, function(err, result) {
                        err && alert("something bad happened contact help@wembli.com"), scope.submitInProgress = !0, 
        ...
        compile: function() {
            return function(scope) {
                plan.get(function(plan) {
                    scope.event = plan.event, scope.venue = plan.venue.data;
                });
            };
        }
    };
} ]).directive("notifyEmail", [ "$rootScope", "$filter", "wembliRpc", "plan", function($rootScope, $filter, wembliRpc) {
    return {
        restrict: "C",
        cache: !1,
        compile: function() {
            return function(scope, element, attr) {
                scope.emailCollected = !1, scope.notifyEmail = "", scope.collectEmail = function() {
                    wembliRpc.fetch("customer.notify", {
                        addOn: attr.addOn,
                        email: scope.notifyEmail
                    }, function() {}), scope.emailCollected = !0;
                };
            };
        }
    };
    ...
    return {
        ...
        compile: function(element) {
            return $window.twttr = function(d, s, id) {
                ...
                if (!d.getElementById(id)) return js = d.createElement(s), js.id = id, js.src = "//platform.twitter.com/widgets.js", 
                element.append(js), $window.twttr || (t = {
                    _e: [],
                    ready: function(f) {
                        t._e.push(f);
                    }
    ...
    return {
        ...
        compile: function() {
            return function(scope, element, attr) {
                rsvpLoginModal.set("guid", attr.guid), rsvpLoginModal.set("service", attr.service), 
                rsvpLoginModal.set("token", attr.token), rsvpLoginModal.set("friend", attr.friend), 
                rsvpLoginModal.set("event", attr.event), rsvpLoginModal.set("confirmSocial", attr.confirmSocial), 
...
} ]).directive("sendConfirmationEmail", [ "wembliRpc", function(wembliRpc) {
    return {
        restrict: "C",
        compile: function() {
            return function(scope, element, attr) {
                scope.confirmationEmailSent = !1, scope.confirmationEmailInProgress = !1, attr.$observe("next", function() {
                    element.click(function() {
                        scope.confirmationEmailInProgress = !0;
                        var rpcArgs = {};
                        attr.next && (rpcArgs.next = attr.next), wembliRpc.fetch("customer.sendConfirmationEmail", rpcArgs, function() {
                            scope.confirmationEmailSent = !0, scope.confirmationEmailInProgress = !1, scope.$broadcast("confirmation-email-sent");
                        });
                    });
                });
            };
        ...
        compile: function() {
            return function(scope, element, attr) {
                attr.$observe("email", function() {
                    element.click(function() {
                        var rpcArgs = {};
                        rpcArgs.email = attr.email, attr.next && (rpcArgs.next = attr.next), wembliRpc.fetch("customer.sendForgotPasswordEmail", rpcArgs, function(err, result) {
                            scope.forgotPasswordEmailSent = !0, err && scope.$broadcast("forgot-password-email-sent", err), 
                            result.error === !0 ? scope.$broadcast("forgot-password-email-sent", {
                                error: !0
                            }) : scope.$broadcast("forgot-password-email-sent");
                        }, function(data) {
                            return scope.accountExists = !1, scope.signupSpinner = !0, data;
                        }, function(data) {
                            return scope.signupSpinner = !1, JSON.parse(data);
                        });
                    });
                });
            };
    ...
    return {
        ...
        compile: function() {
            return function(scope, element, attr) {
                element.click(function() {
                    var nextLink = "";
                    nextLink = attr.href ? attr.href : element.find(".next-link").attr("href"), $rootScope.$broadcast("payment-type-modal-clicked", {
                        nextLink: nextLink,
                        name: attr.name,
...
} ]).directive("highSalesPerformers", [ "wembliRpc", function(wembliRpc) {
    return {
        restrict: "E",
        compile: function() {
            var lookup = {
...
} ]).directive("showEllipses", [ function() {
    return {
        restrict: "C",
        compile: function() {
            return function(scope, elm, attrs) {
                var shorten = function(t) {
                    if (t.length > attrs.characters) {
                        var shortened = t.substr(attrs.start, attrs.characters), shortened = shortened + "...";
                        elm.text(shortened);
                    }
                };
                attrs.interpolate ? attrs.$observe("text", function() {
                    shorten(attrs.text);
                }) : shorten(elm.text());
            };
        }
    };
} ]).directive("wembliHide", [ function() {
    return {
        restrict: "EAC",
        compile: function() {
            return function(scope, element, attr) {
...
angular.module("wembliApp.services", []).factory("loadingModal", [ "$rootScope", function($rootScope) {
    var stack = 0;
    return {
        title: "Page Loading",
        show: function(title, body) {
            title && (this.title = title), body && (this.body = body), 0 == stack && angular.element(".modal").modal("hide"), 
            stack++, angular.element("#generic-loading-modal").modal("show"), $rootScope.$broadcast("loading-modal-show");
        },
        hide: function() {
            stack--, 1 > stack && (stack = 0, "undefined" != typeof $("#generic-loading-modal").modal && $("#generic-loading-modal").modal("hide"));
        }
    };
} ]).factory("environment", [ "$location", function($location) {
    var env = "development";
    return /www/.test($location.host()) && (env = "production"), env;
} ]).factory("balancedApiConfig", [ "environment", function(environment) {
    var config = {
        development: {
    ...
    return envConfig.balancedMarketplaceUri = "/v1/marketplaces/" + envConfig.balancedMarketplace, 
    envConfig;
} ]).factory("tnConfig", [ "environment", function(environment) {
    var config = {
        development: {
    ...
    return envConfig.url = envConfig.baseUrl + "?brokerid=" + envConfig.brokerId + "&sitenumber=" + envConfig.siteNumber, 
    envConfig;
} ]).factory("ticketPurchaseUrls", [ "tnConfig", function(tnConfig) {
    return {
        tn: tnConfig.url
    };
} ]).factory("initRootScope", [ "$window", "$rootScope", "$location", function($window, $rootScope) {
    $rootScope.getDate = function(d) {
        var tmpDate = new Date(), h = "0" + tmpDate.getTimezoneOffset() / 60;
        h = h.substr(-2);
        var m = "0" + tmpDate.getTimezoneOffset() % 60;
        m = m.substr(-2);
        var operator = tmpDate.getTimezoneOffset() > 0 ? "-" : "+", offset = operator + h + ":" + m;
        return /Z$/.test(d) && (d = d.substring(0, d.length - 1)), d += offset, new Date(d);
    };
} ]).factory("pluralize", [ "$rootScope", "wembliRpc", "customer", function() {
    return function(num) {
        return 1 != num;
    };
    ...
    return {
        deal: function(number) {
            return 1 == number ? "deal" : "deals";
        },
        spot: function(number) {
            return 1 == number ? "spot" : "spots";
        },
        person: function(number) {
            return 1 == number ? "person" : "people";
        },
        ticket: function(number) {
            return 1 == number ? "ticket" : "tickets";
        },
        parking: function(number) {
            return 1 == number ? "parking" : "parking";
        },
        restaurant: function(number) {
            return 1 == number ? "restaurant" : "restaurants";
        },
        hotel: function(number) {
            return 1 == number ? "hotel" : "hotels";
        },
        seat: function(number) {
            return 1 == number ? "seat" : "seats";
        },
        guest: function(number) {
            return 1 == number ? "guest" : "guests";
        }
    };
} ]).factory("plan", [ "$rootScope", "wembliRpc", "customer", "$timeout", "loggedIn", function($rootScope, wembliRpc, customer, $timeout, loggedIn) {
    var self = this;
    return self.plan = null, self.tickets = null, self.fetchInProgress = !1, self.getStack = 0, 
    self.fetchStack = 0, {
        get: function(callback) {
            if (!callback) return self.plan;
            if (self.plan) callback(self.plan); else if (0 == self.getStack) this.fetch(function(p) {
                callback(p.plan);
            }); else {
                self.getStack++;
                var dereg = $rootScope.$on("plan-fetched", function() {
                    self.getStack--, 0 == self.getStack && dereg(), callback(self.plan);
                });
            }
        },
        set: function(plan, friends, organizer) {
            return "undefined" != typeof plan && (self.plan = plan), "undefined" != typeof friends && (self.friends = friends), 
            "undefined" != typeof organizer && (self.organizer = organizer), self.plan;
        },
        setFriends: function(friends) {
            return self.friends = friends, self.friends;
        },
        getFriends: function() {
            return self.friends;
        },
        ...
        setTickets: function(tickets) {
            return self.tickets = tickets, self.tickets;
        },
        getParking: function() {
            return self.parking;
        },
        setParking: function(parking) {
            return self.parking = parking, self.parking;
        },
        getHotels: function() {
            return self.hotels;
        },
        setHotels: function(hotels) {
            return self.hotels = hotels, self.hotels;
        },
        getRestaurants: function() {
            return self.restaurants;
        },
        setRestaurants: function(restaurants) {
            return self.restaurants = restaurants, self.restaurants;
        },
        getFeed: function() {
            return self.feed;
        },
        setFeed: function(feed) {
            return self.feed = feed, self.feed;
        },
        getOrganizer: function() {
            return self.organizer;
        },
        setOrganizer: function(o) {
            return self.organizer = o, self.organizer;
        },
        getContext: function() {
            return self.context;
        },
        setContext: function(c) {
            return self.context = c, self.context;
        },
        addFriend: function(args, callback) {
            wembliRpc.fetch("plan.addFriend", args, function(err, result) {
                result.friends && (self.friends = result.friends, $rootScope.$broadcast("plan-friends-changed", result.friends)), 
                callback(err, result);
            });
        },
        ...
        fetch: function(args, callback) {
            if ("undefined" == typeof callback && (callback = args, args = {}), self.fetchInProgress) {
                if (callback) {
                    $rootScope.$on("plan-fetched", function() {
                        callback(self);
                    });
                }
            } else self.fetchInProgress = !0, wembliRpc.fetch("plan.init", args, function(err, result) {
                "undefined" != typeof result.plan && ($rootScope.plan = result.plan, self.plan = result.plan, 
                self.friends = result.friends, self.tickets = result.tickets, self.parking = result.parking, 
                self.hotels = result.hotels, self.restaurants = result.restaurants, self.organizer = result.organizer, 
                self.context = result.context, self.feed = result.feed), "undefined" != typeof result.loggedIn && ($rootScope.loggedIn = result.loggedIn, 
                loggedIn.set(result.loggedIn)), "undefined" != typeof result.rememberEmail && ($rootScope.rememberEmail = result.rememberEmail), 
                "undefined" != typeof result.customer && result.customer && customer.set(result.customer), 
                self.fetchInProgress = !1, $rootScope.$broadcast("plan-fetched", {}), callback && callback(self);
            });
        },
        poll: function(callback) {
            ...
            !function tick() {
                p.fetch({
                    ...
                    callback(p), $timeout(tick, 3e4);
                });
            }();
        },
        save: function(callback) {
            wembliRpc.fetch("plan.save", {}, function(err, result) {
                err || ($rootScope.$broadcast("plan-saved", {}), self.plan = result.plan, callback && callback(err, result));
            });
        },
        submitRsvp: function(rsvpFor, args, callback) {
            args.rsvpFor = rsvpFor, wembliRpc.fetch("plan.submitRsvp", args, function(err, result) {
                return $("#generic-loading-modal").modal("hide"), callback ? callback(err, result) : void 0;
            }, function(data) {
                return $rootScope.genericLoadingModal.header = "Saving...", $("#page-loading-modal").modal("hide"), 
                $("#generic-loading-modal").modal("show"), data;
            }, function(data) {
                return JSON.parse(data);
            });
        },
        rsvpComplete: function() {
            var complete = !0;
            if ("undefined" == typeof self.plan.rsvpDate) return !complete;
            var now = new Date();
            now.setHours(0, 0, 0, 0);
            var t1 = now.getTime(), rsvpDate = new Date(self.plan.rsvpDate);
            rsvpDate.setHours(24, 0, 0, 0);
            var t2 = rsvpDate.getTime();
            if (t1 >= t2) return complete;
            if (0 == self.friends.length) return !complete;
            for (var i = 0; i < self.friends.length; i++) if (null === self.friends[i].rsvp.decision) return !complete;
            return complete;
        },
        ponyUpSent: function() {
            if (!self.plan.ponyUpDate) return !1;
            var now = new Date();
            now.setHours(0, 0, 0, 0);
            var t1 = now.getTime(), ponyUpDate = new Date(self.plan.ponyUpDate);
...
} ]).factory("customer", [ "$rootScope", function($rootScope) {
    var self = this;
    return self.customer = null, {
        ...
        set: function(customer) {
            return $rootScope.customer = self.customer = customer, $rootScope.$broadcast("customer-changed", self.customer), 
            self.customer;
        }
    };
} ]).factory("fetchModals", [ "$rootScope", "$location", "$http", "$compile", function($rootScope, $location, $http, $compile) {
    var modalFetched = {}, modalFetchInProgress = null, success = function(callback) {
        $rootScope.$broadcast($("body :first-child").attr("id") + "-fetched", {
            modalId: $("body :first-child").attr("id")
        }), callback && callback();
    }, fetchPartial = function(partialUrl, callback) {
        modalFetchInProgress = partialUrl, $http({
            method: "get",
            url: partialUrl,
            cache: !1
        }).success(function(data) {
            return $("body").prepend($compile(data)($rootScope)), modalFetched[partialUrl] = $("body :first-child").attr("id"), 
            modalFetchInProgress = null, callback && callback(), success();
        }).error(function(err) {
    ...
    return {
        fetch: function(path, callback) {
            ...
            if (modalFetched[pathKey]) return success(callback);
            {
                if (modalFetchInProgress !== pathKey) return /^\/partials/.test(path) ? fetchPartial(path, callback) : void 0;
                if (callback) var dereg = $rootScope.$on($("body :first-child").attr("id") + "-fetched", function() {
                    callback(), dereg();
                });
            }
        }
...
} ]).factory("googleAnalytics", [ "wembliRpc", "cookie", function(wembliRpc, cookie) {
    var self = this;
    self.cookie = {
        __utmz: {}
    };
    ...
    if (cookie) {
        var z = cookie.split(".");
        if (z.length >= 4) for (var y = z[4].split("|"), i = 0; i < y.length; i++) {
            var pair = y[i].split("=");
            self.cookie.__utmz[pair[0]] = pair[1];
        }
    }
    return console.log("google cookie values"), console.log(self.cookie), {
        getCookie: function() {
            return self.cookie;
        },
        trackEvent: function(category, action, label, value, cb) {
            _gaq.push([ "_trackEvent", category, action, label, value ]), wembliRpc.fetch("analytics.addEvent", {
                collection: "event",
                category: category,
                action: action,
                label: label,
                ...
                "undefined" != typeof cb && cb(err, result);
            });
        }
    };
    ...
    return wembliRpc.fetch = function(method, args, callback, transformRequest, transformResponse) {
        var cache = args.cache ? !0 : !1;
        cache && delete args.cache, jsonRpc.method = method, jsonRpc.params.args = args;
        var data = JSON.stringify(jsonRpc), cb = function(err, result) {
            ...
            if (err) {
                ...
                return $rootScope.$broadcast(eventName + "callbackComplete", {
                    method: method
                }), void 0;
            }
            $rootScope.$broadcast(eventName + "success", {
                method: method
            }), "undefined" != typeof result.customer && (customer.set(result.customer), $rootScope.$broadcast("customer-fetched", {
                customer: customer
            })), "undefined" != typeof result.loggedIn && ($rootScope.loggedIn = result.loggedIn, 
        ...
        return cache && "undefined" != typeof wembliRpc._cache[data] ? cb(null, wembliRpc._cache[data]) : ("undefined" == typeof callback && (callback = function() {
            return !0;
        }), "undefined" == typeof transformRequest && (transformRequest = function(data) {
            return data;
        }), "undefined" == typeof transformResponse && (transformResponse = function(data) {
            return JSON.parse(data);
        }), $http.post("/", data, {
            headers: {
            ...
            var result = {};
            return result.data = data, result.status = status, cache && (wembliRpc._cache[data] = result.data.result), 
            result.data.error ? cb(result.data.error) : cb(null, result.data.result);
        }).error(function(err) {
            callback(err);
        }));
    }, wembliRpc;
} ]).value("version", "0.0.1"), angular.module("wembliApp.filters", []).filter("pluralize", [ "$filter", "pluralizeWords", function($filter, pluralizeWords) {
    return function(number, word) {
        return pluralizeWords[word](number);
    };
} ]).filter("sortNumeric", [ function() {
    return function(elements, predicate, reverse) {
        return "undefined" != typeof elements ? (elements.sort(function(a, b) {
            return reverse ? a[predicate] - b[predicate] : b[predicate] - a[predicate];
        }), elements) : void 0;
    };
} ]), angular.module("wembliApp.controllers.header", []).controller("HeaderCtrl", [ "$scope", "$aside", function($scope) {
    $scope.aside = {
        title: "Title",
        content: "Content"
    };
} ]), angular.module("wembliApp.services.header", []).factory("header", [ function() {
    var self = this;
    return {
        init: function(header, scroll) {
            header = header || "#header", scroll = scroll || "window", "window" === scroll && (scroll = window), 
            self.prevScroll = angular.element(scroll).scrollTop(), angular.element(header).show(), 
            angular.element(scroll).on("scroll", function() {
                return angular.element(scroll).scrollTop() < 1 ? (angular.element(header).show(), 
                void 0) : angular.element(scroll).height() < angular.element(scroll).scrollTop() ? (angular.element(header).hide(), 
                void 0) : (angular.element(scroll).scrollTop() > self.prevScroll ? angular.element(header).is(":visible") && angular.element(header).fadeOut(250) : angular.element(header).is(":visible") || angular.element(header).fadeIn(250), 
                self.prevScroll = angular.element(scroll).scrollTop(), void 0);
            });
        },
        fixed: function() {}
    };
} ]), angular.module("wembliApp.services.facebook", []).factory("facebook", [ "$rootScope", "$q", "wembliRpc", "$window", "$filter", "customer", "$location", function($rootScope, $q, wembliRpc, $window, $filter, customer, $location) {
    var self = this;
    return this.auth = null, this.friends = null, this.allFriends = null, self.preApi = {}, 
    self.postApi = {
        "/me/friends": function(response, cb) {
            "undefined" != typeof response.data ? (self.friends = $filter("orderBy")(response.data, "+name"), 
            self.allFriends = self.friends) : self.friends = null, cb(response);
        }
    }, {
        feedDialog: function(args, cb) {
            FB.getLoginStatus(function(response) {
                if (response.authResponse) {
                    var actionLink = "http://" + $location.host() + "/rsvp/" + args.guid + "/" + args.token + "/facebook", obj = {
                        method: "feed",
                        display: "iframe",
                        link: actionLink,
        ...
        getLoginStatus: function() {
            FB.getLoginStatus(function(response) {
                "connected" === response.status ? (self.auth = response.authResponse, wembliRpc.fetch("facebook.setAccessToken", {
                    accessToken: response.authResponse.accessToken
                }, function(err) {
                    return err ? !1 : void 0;
                })) : self.auth = !1, $rootScope.$broadcast("facebook-login-status", {
                    auth: self.auth
                });
            });
        },
        ...
        login: function() {
            FB.login(function(response) {
                response.authResponse && (self.auth = response.authResponse, $rootScope.$broadcast("facebook-login", {
                    auth: self.auth
                }));
            });
        },
        logout: function() {
            self.auth ? FB.logout(function(response) {
                response && (self.auth = !1, $rootScope.$broadcast("facebook-logout", {
                    auth: self.auth
                })), $window.location = "/logout";
            }) : $window.location = "/logout";
        },
        api: function(method, callback) {
            "undefined" != typeof self.preApi[method] && self.preApi[method](), FB.api(method, function(response) {
                $rootScope.$broadcast("facebook-api-response", {
                    method: method,
                    response: response
                }), "undefined" != typeof self.postApi[method] ? self.postApi[method](response, callback) : callback(response);
            });
        }
    };
} ]), angular.module("wembliApp.services.twitter", []).factory("twitter", [ "$rootScope", "$filter", "wembliRpc", function($rootScope, $filter, wembliRpc) {
    var self = this;
    return this.auth = null, this.selectedFriends = {}, this.friends = null, this.allFriends = null, 
    {
        tweet: function(args, cb) {
            wembliRpc.fetch("twitter.tweet", args, function(err, result) {
                return err ? cb(err) : ($rootScope.$broadcast("twitter-tweet", {}), cb(err, result), 
                void 0);
            });
        },
        getAuth: function() {
        ...
        getLoginStatus: function() {
            wembliRpc.fetch("twitter.getLoginStatus", {}, function(err, result) {
                return err ? !1 : (self.auth = result.loginStatus, $rootScope.$broadcast("twitter-login-status", {
                    auth: self.auth
                }), void 0);
            });
        },
        searchUsers: function(q, args, callback) {
            wembliRpc.fetch("twitter.searchUsers", {
                ...
                err && callback(!1), self.friends = $filter("orderBy")(result.friends, "+name"), 
                self.allFriends = self.friends, callback(result);
            });
        },
        getFriends: function() {
            return self.friends;
        ...
        fetchFriends: function(callback) {
            wembliRpc.fetch("twitter.getFriends", {}, function(err, result) {
                err && callback(!1), self.friends = result.friends.users, self.allFriends = result.friends.users, 
                callback(result);
            });
        },
...
} ]), angular.module("wembliApp.services.pixel", []).factory("pixel", [ "$http", "wembliRpc", function($http, wembliRpc) {
    var self = this;
    return self.pixelsFired = {}, {
        fire: function(args, cb) {
            if ("undefined" == typeof self.pixelsFired[args.content]) {
                console.log("pixel args: "), console.log(args);
                var getArgs = {
                    method: "get",
                    cache: !1,
                    url: "/partials/pixel/" + args.source + "/" + args.content
                };
                $http(getArgs).success(function(data) {
                    $(data).insertBefore($("script")[0]), self.pixelsFired[args.content] = !0, wembliRpc.fetch("analytics.addEvent", {
...
} ]), angular.module("wembliApp.directives.header", []).directive("wembliHeader", [ "header", function(header) {
    return {
        restrict: "C",
        replace: !1,
        compile: function() {
            return function(scope, element, attr) {
                attr.header = attr.header || "#header", attr.scroll = attr.scroll || "window", header.init(attr.header, attr.scroll);
            };
        }
    };
} ]);