...
!function($) {
...
} ]), angular.module("wembliApp.controllers.search", []).controller("EventListCtrl", [ "$scope", "$location", "wembliRpc", "$filter", "$rootScope", "plan", "fetchModals", "loadingModal", "$timeout", function() {} ]).controller("SearchCtrl", [ "$scope", "$rootScope", "$window", "wembliRpc", "googleAnalytics", function($scope, $rootScope, $window, wembliRpc, googleAnalytics) {
    ...
    $scope.$on("search-page-loaded", function() {
        ...
        console.log(eventData), $scope.eventData = eventData, $("#payment-type-modal").modal("show");
    }, $scope.startPlan = function() {
        console.log("clicked startPlan"), console.log($scope.eventData), $scope.submitInProgress = !0, 
        "split-first" === $scope.paymentType && ($scope.nextLink = "/event-options/" + $scope.eventData.ID + "/" + $scope.eventData.Name), 
        ...
        wembliRpc.fetch("plan.startPlan", {
            ...
            return console.log(result), result.success ? (googleAnalytics.trackEvent("Plan", "start", $scope.eventName, "", function() {
                $scope.submitInProgress = !1, $window.location.href = $scope.nextLink;
            }), void 0) : ($("#no-event-modal").modal("show"), $("#payment-type-modal").modal("hide"), 
            void 0);
        });
    };
