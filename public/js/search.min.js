        ...
        return utils.mixin(Dataset.prototype, {
            ...
            _getLocalSuggestions: function(terms) {
                ...
                return utils.each(terms, function(i, term) {
                    ...
                    return list ? (lists.push(list), (!shortestList || list.length < shortestList.length) && (shortestList = list), 
                    void 0) : !1;
        ...
        return utils.mixin(DropdownView.prototype, EventTarget, {
            ...
            _moveCursor: function(increment) {
                ...
                if (this.isVisible()) {
                    ...
                    nextIndex = (nextIndex + 1) % ($suggestions.length + 1) - 1, -1 === nextIndex) return this.trigger("cursorRemoved"), 
                    void 0;
            ...
            setLanguageDirection: function(dir) {
                ...
                "ltr" === dir ? this.$menu.css(ltrCss) : this.$menu.css(rtlCss);
            },
...
} ]), angular.module("wembliApp.controllers.search", []).controller("EventListCtrl", [ "$scope", "$location", "wembliRpc", "$filter", "$rootScope", "plan", "fetchModals", "loadingModal", "$timeout", function() {} ]).controller("SearchCtrl", [ "$scope", "$rootScope", "$window", function($scope, $rootScope) {
    $rootScope.$broadcast("search-page-loaded", {}), $scope.searchInProgress = !1, $scope.$on("search-page-loaded", function() {
        $scope.searchInProgress = !1;
    }), $scope.submitSearch = function() {
        $scope.searchInProgress = !0;
    }, $scope.openPaymentModal = function(event) {
        console.log(event), $scope.event = event, $("#payment-type-modal").modal("show");
    };
} ]).controller("PaymentTypeModalCtrl", [ "$scope", "$location", "plan", "wembliRpc", "$rootScope", "googleAnalytics", function($scope, $location, plan, wembliRpc, $rootScope, ga) {
    $scope.$on("payment-type-modal-clicked", function(e, args) {
        $scope.$apply(function() {
            $scope.name = args.name, $scope.eventId = args.eventId, $scope.eventName = args.eventName, 
            $scope.nextLink = args.nextLink;
        });
    }), $scope.submitInProgress = !1, $scope.startPlan = function() {
        $scope.submitInProgress = !0, "split-first" === $scope.paymentType && ($scope.nextLink = "/event-options/" + $scope.eventId + "/" + $scope.eventName), 
        "undefined" == typeof $scope.nextLink && ($scope.nextLink = "/tickets/" + $scope.eventId + "/" + $scope.eventName), 
        wembliRpc.fetch("plan.startPlan", {
            payment: $scope.paymentType,
            eventId: $scope.eventId,
            eventName: $scope.eventName
        }, function(err, result) {
            console.log(result), ga.trackEvent("Plan", "start", $scope.eventName), plan.fetch(function() {
                $scope.submitInProgress = !1, $location.path($scope.nextLink);
            });
        });
    };
                ...
                var opts = {
                    ...
                    if ("down" === direction) {
                        ...
                        wembliRpc.fetch(method, args, function(err, result) {
                            return scope.loadingMoreEvents = !1, err ? (scope.noMoreEvents = !0, $this.waypoint("destroy"), 
        ...
        compile: function() {
            return function(scope, element, attr) {
                element.mouseover(function() {
                    ...
                    "undefined" != typeof scope.ticketSummaryData[attr.eventId]) return console.log("already fetched"), 
                    void 0;
                    if (scope.ticketSummaryData[attr.eventId] = {}, scope.ticketSummaryData[attr.eventId].locked) return console.log("locked"), 
                    void 0;
                    ...
                    console.log("getpricing info"), console.log(args), wembliRpc.fetch("event.getPricingInfo", args, function(err, result) {
                        ...
                        return err ? ($("#" + attr.id + " .ticket-range").html(summaryContent).fadeIn(), 
                        void 0) : (scope.ticketSummaryData[attr.eventId].locked = !1, "undefined" != typeof result.ticketPricingInfo.ticketsAvailable && "0" !== result.ticketPricingInfo.ticketsAvailable && (summaryContent = "1" === result.ticketPricingInfo.ticketsAvailable ? result.ticketPricingInfo.ticketsAvailable + " ticket" : result.ticketPricingInfo.ticketsAvailable + " tickets", 
                        ...
                        scope.ticketSummaryData[attr.eventId] = summaryContent, summaryContent && $("#" + attr.id + " .ticket-range").html(summaryContent).fadeIn(), 
                        void 0);
                    });
                });
            };
