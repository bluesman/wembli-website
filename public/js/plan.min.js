...
angular.module("wembliApp", [ "ngRoute", "mgcrea.ngStrap", "wembliApp.controllers", "wembliApp.controllers.plan", "wembliApp.controllers.header", "wembliApp.filters", "wembliApp.filters.plan", "wembliApp.filters.invitation-wizard", "wembliApp.services", "wembliApp.services.header", "wembliApp.services.facebook", "wembliApp.services.twitter", "wembliApp.services.pixel", "wembliApp.services.plan", "wembliApp.services.google", "wembliApp.directives", "wembliApp.directives.header", "wembliApp.directives.plan" ]).config([ "$routeProvider", "$locationProvider", function() {} ]).run([ "$timeout", "$rootScope", "initRootScope", "$window", "facebook", "twitter", "plan", "overlay", "planNav", function($timeout, $scope, initRootScope, $window, facebook, twitter, plan, overlay, planNav) {
    console.log("run plan angular app"), overlay.loading(!0), overlay.show(), planNav.onActivate(function() {
...
} ]).controller("PlanCtrl", [ "$scope", "wembliRpc", "$window", "plan", "planNav", "$location", "$rootScope", "googleMap", function($scope, wembliRpc, $window, plan, planNav, $location, $rootScope, googleMap) {
    plan.get(function(p) {
        $scope.plan = p, $scope.activateSection = function(sectionNumber) {
            console.log("activateSection " + sectionNumber);
            var sectionNumber = parseInt(sectionNumber.charAt(sectionNumber.length - 1));
            planNav.activate(sectionNumber), googleMap.resize();
        };
...
} ]).directive("planDashboard", [ "$templateCache", "$timeout", "$rootScope", "$window", "$location", "wembliRpc", "cart", "plan", "customer", "pluralize", "fetchModals", "planNav", function($templateCache, $timeout, $rootScope, $window, $location, wembliRpc, cart, plan, customer, pluralize) {
    return {
        restrict: "C",
        replace: !0,
        scope: !1,
        controller: [ "$scope", "$element", "$attrs", "$transclude", function($scope) {
            $scope.friendsPonyUp = function(friends) {
                ...
                if (console.log("here"), $scope.totalComing = 0, $scope.friendsComing = [], $scope.totalPoniedUp = 0, 
                $scope.friends) {
            ...
            }, $scope.$on("plan-friends-changed", function() {
                $scope.plan = plan.get(), $scope.friends = plan.getFriends(), $scope.calcTotalComing();
        ...
        compile: function() {
            return function(scope) {
                $templateCache.removeAll(), scope.serviceFee = function(price) {
                    return .15 * price;
...
} ]).directive("organizerPlanDashboard", [ "$rootScope", "$window", "$location", "wembliRpc", "cart", "plan", "customer", "pluralize", "fetchModals", "planNav", function($rootScope, $window, $location, wembliRpc, cart, plan, customer, pluralize, fetchModals) {
    return {
        restrict: "C",
        scope: !1,
        controller: [ "$scope", "$element", "$attrs", "$transclude", function($scope) {
            fetchModals.fetch("/partials/tickets-offsite", function(err) {}), $scope.$watch("plan.tickets[0].ticketGroup.selectedQty", function() {
                "undefined" != typeof newVal && oldVal !== newVal && (cart.totals("tickets"), scope.friendsPonyUp(scope.friends), 
                $scope.reconcileTicketQty());
                ...
                plan.savePreferences({
                    ...
                    $scope.plan = result.plan, $scope.calcTotalComing(), "undefined" != typeof cb && cb();
                });
                ...
                "" !== $scope.plan.organizer.rsvp.guestCount && ($scope.plan.organizer.rsvp.decision = $scope.plan.organizer.rsvp.guestCount > 0, 
            ...
            var initDirective = function() {
                null === $scope.plan.organizer.rsvp.decision && $scope.setRsvp(!0), $scope.calcTotalComing();
            };
            if ($scope.plan) initDirective(); else var d = $scope.$watch("plan", function(newVal) {
                newVal && (initDirective(), d());
            });
        } ],
        compile: function() {
            return function(scope) {
                    ...
                    plan.get(function(p) {
                        if (!p.rsvpComplete) {
                            var handleRsvpComplete = function() {
                                p.rsvpComplete || ($("#rsvp-complete-notification").show(), wembliRpc.fetch("plan.submitRsvpComplete", {
                                    rsvpComplete: !0
                                }, function(err, result) {
                                    plan.set(result.plan), scope.plan = result.plan, $rootScope.$broadcast("plan-changed", {}), 
                                    $rootScope.$broadcast("rsvp-complete", {});
                                }));
                            };
                            if (plan.rsvpComplete()) handleRsvpComplete(); else var deregRsvpComplete = scope.$watch("plan", function(newVal) {
                                newVal && plan.rsvpComplete() && (handleRsvpComplete(), deregRsvpComplete());
                            });
                        }
                    });
                });
...
} ]).directive("organizerItinerarySection", [ "wembliRpc", "planNav", function(wembliRpc, planNav) {
    return {
        restrict: "E",
        cache: !1,
        replace: !0,
...
} ]).directive("organizerRsvpSection", [ "planNav", "plan", function(planNav, plan) {
    return {
        restrict: "E",
        cache: !1,
        replace: !0,
        ...
        compile: function() {
            return function(scope) {
                ...
                plan.get(function() {
                    ...
                    newVal && newVal !== oldVal && makeRsvpDays();
                });
            };
        }
    };
} ]).directive("organizerCartSection", [ "ticketPurchaseUrls", "plan", "cart", "planNav", function(ticketPurchaseUrls, plan, cart, planNav) {
    return {
        restrict: "E",
        replace: !0,
        scope: !1,
...
} ]).directive("organizerPonyUpSection", [ "$rootScope", "plan", "wembliRpc", "$timeout", "customer", "planNav", function($rootScope, plan, wembliRpc, $timeout, customer, planNav) {
    return {
        restrict: "E",
        cache: !1,
        replace: !0,
...
} ]), angular.module("wembliApp.services.plan", []).factory("planNav", [ "$timeout", "$rootScope", "$location", "header", "notifications", "overlay", function($timeout, $rootScope, $location, header, notifications) {
    var self = this;
    self.sectionsCount = 0, self.sectionsLoaded = 0, self.activateSection = 1, self.arrowTop = 111, 
    self.arrowHeight = 109;
    var planNav = {
        registerSection: function() {
            self.sectionsLoaded++, this.activate();
        },
        onActivate: function(f) {
            self.onActivate = f;
        },
        setActivateSection: function(sectionNumber) {
            console.log("to section " + sectionNumber), self.activateSection = sectionNumber, 
            this.activate();
        },
        setSectionsCount: function(cnt) {
            return self.sectionsCount = cnt, this.activate(), cnt;
        },
        getSectionsCount: function() {
            return self.sectionsCount;
        },
        activate: function(sectionNumber) {
            if (self.activateSection = sectionNumber ? sectionNumber : self.activateSection, 
            console.log("activate section " + self.activateSection), self.sectionsCount && self.sectionsLoaded == self.sectionsCount) {
                console.log("all sections loaded"), $(".plan-section").removeClass("hide").hide(), 
        ...
        update: function() {
            $timeout(function() {
                $("section").each(function(sectionNumber, section) {
                    var count = 0, key = "#" + section.id + " .notification";
                    $(key).each(function(idx, el) {
                        "block" === $(el).css("display") && count++;
                    }), count ? $("#notification-nav-" + section.id).html(count).show() : $("#notification-nav-" + section.id).html(count).hide();
                });
            }, 500);
        }
    };
