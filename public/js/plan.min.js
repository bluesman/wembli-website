...
!function($, undefined) {
    ...
    overlay.loading(!0), overlay.show(), planNav.onActivate(function() {}), $timeout(function() {
        plan.fetch(function() {});
    ...
    $scope.getPlacePhotoUrl = function(type, idx) {
        ...
        "undefined" != typeof newVal && oldVal !== newVal && (cart.totals("tickets"), $scope.setSelectedQty());
        ...
        overlay.loading(!1), overlay.hide(), plan.poll(function(plan) {
            plan.get(function(p) {
                $scope.plan = p, $scope.organizer = plan.getOrganizer(), $scope.tickets = plan.getTickets(), 
                $scope.parking = plan.getParking(), $scope.hotels = plan.getHotels(), $scope.restaurants = plan.getRestaurants(), 
                $scope.friends = plan.getFriends(), $scope.context = plan.getContext(), $scope.feed = plan.getFeed(), 
                $scope.calcTotalComing(), $scope.setSelectedQty(), $scope.reconcileTicketQty();
            });
        });
    });
    ...
    $scope.tnUrl = ticketPurchaseUrls.tn, $scope.$watch("plan.tickets[0].ticketGroup.selectedQty", function() {
        "undefined" != typeof newVal && oldVal !== newVal && (cart.totals("tickets"), $scope.reconcileTicketQty());
    ...
    planNav.activate("cart"), $scope.showChangeTicketsLink = function() {
        plan.get(function(p) {
    ...
    var submitVote = function() {
        ...
        var total = 0;
        console.log($scope.me.rsvp.tickets.price), parseInt($scope.me.rsvp.tickets.price) > 0 && (total += parseInt($scope.me.rsvp.tickets.price)), 
        $scope.me.rsvp.parking.decision && $scope.plan.preferences.addOns.parking && parseInt($scope.me.rsvp.parking.price) > 0 && (total += parseInt($scope.me.rsvp.parking.price)), 
        $scope.me.rsvp.restaurant.decision && $scope.plan.preferences.addOns.restaurants && parseInt($scope.me.rsvp.restaurant.price) > 0 && (total += parseInt($scope.me.rsvp.restaurant.price)), 
        $scope.me.rsvp.hotel.decision && $scope.plan.preferences.addOns.hotels && parseInt($scope.me.rsvp.hotel.price) > 0 && (total += parseInt($scope.me.rsvp.hotel.price)), 
        $scope.votePriceTotalPerPerson = total, $scope.votePriceTotal = total * ($scope.me.rsvp.guestCount + 1);
    };
    $scope.$watch("me.rsvp.parking.decision", function(val) {
        "undefined" != typeof val && (submitVote(), calcVotePriceTotal());
    ...
    $scope.displaySendPonyUp = !1, $scope.showFees = !1, $scope.showHistory = !1, $scope.$watch("ponyUp.amountFormatted", function(newVal) {
...
} ]).controller("FriendItineraryCtrl", [ "$scope", "plan", "planNav", "wembliRpc", function($scope, plan, planNav) {
    planNav.activate("itinerary");
            ...
            $timeout(function() {
                ...
                plan.get(function(p) {
                    angular.forEach(p.notifications, function(n) {
                        ...
                        "undefined" == typeof n || n.acknowledged ? (console.log("hiding notification because it has been acknowledged: "), 
                        $(el).hide()) : $(el).show();
    ...
    return self.tickets = {
        ...
        parking: {
            ...
            getQty: function(item) {
                return item.purchased ? item.payment.qty || 0 : "pw" === item.service ? item.parking.available_spots : 0;
            },
            totalEach: function(item, price, fee) {
                return item.purchased ? price : price + fee;
            },
        ...
        suggestedAmounts: function() {
            var friends = plan.getFriends(), p = plan.get();
            if ("undefined" != typeof p && "undefined" != typeof friends) {
                ...
                angular.forEach(addOns, function(a) {
                    ...
                    var funcs = self.methods[a], config = funcs.getConfig(), items = funcs.get(), remainingQty = null, remainingItem = null, used = {};
                    used[a] = {};
                    var handleFriend = function(friend) {
                        var friendCount = 1, purchased = [], notPurchased = [];
                        if (friend[a] = {}, friend[a].suggestedPonyUpAmount = 0, friend.suggestedPonyUpAmount = 0, 
                        config.perGroup || (friendCount += parseInt(friend.rsvp.guestCount)), console.log("friendCount: " + friendCount), 
                        console.log("remainingQty: " + remainingQty), remainingQty > 0) {
                            if (remainingQty >= friendCount) return remainingQty -= friendCount, void (friend[a].suggestedPonyUpAmount += remainingItem.costBreakdown.totalEach * friendCount + remainingItem.costBreakdown.deliveryFeeEach);
                            ...
                            friendCount -= remainingQty, used[a][remainingItem._id] = !0, remainingQty = null, 
                            remainingItem = null;
                        }
                        0 == remainingQty && (used[a][remainingItem._id] = !0, remainingQty = null, remainingItem = null), 
                        angular.forEach(items, function(i) {
                            i.purchased ? purchased.push(i) : notPurchased.push(i);
                        });
                        var all = purchased.concat(notPurchased);
                        angular.forEach(all, function(item) {
                            if ("undefined" != typeof item.costBreakdown && 0 != friendCount && !used[a][item._id]) {
                                var qty = funcs.getQty(item);
                                console.log("item qty: " + qty), qty > friendCount && (remainingQty = qty - friendCount, 
                                remainingItem = item, friend[a].suggestedPonyUpAmount += item.costBreakdown.totalEach * friendCount + item.costBreakdown.deliveryFeeEach, 
                                friendCount = 0), friendCount >= qty && (friendCount -= qty, friend[a].suggestedPonyUpAmount += item.costBreakdown.totalEach * qty + item.costBreakdown.deliveryFeeEach, 
                                used[a][item._id] = !0);
                            }
                        });
                    };
                    p.organizer.rsvp.decision && handleFriend(p.organizer), angular.forEach(friends, handleFriend);
                }), angular.forEach(friends, function(friend) {
                    angular.forEach(addOns, function(a) {
                        friend.suggestedPonyUpAmount += friend[a].suggestedPonyUpAmount, friend[a].suggestedPonyUpAmount = parseFloat(friend[a].suggestedPonyUpAmount).toFixed(2);
        ...
        totals: function(key) {
            var me = this;
            plan.get(function(p) {
                var funcs = self.methods[key], config = funcs.getConfig(), items = funcs.get(), groupTotal = 0, groupCount = 0, groups = [], groupTotalEach = {}, fee = config.fee || 0, deliveryFee = config.deliveryFee || 0, splitBy = 0, deliverySplitBy = 0, remainingSplitBy = null;
                p.organizer.rsvp.decision && (splitBy += 1, config.perGroup || (splitBy += p.organizer.rsvp.guestCount), 
                deliverySplitBy++);
                for (var friends = plan.getFriends(), i = 0; i < friends.length; i++) {
                    var f = friends[i];
                    f.inviteStatus && f.rsvp.decision && (splitBy += 1, config.perGroup || (splitBy += parseInt(f.rsvp.guestCount)), 
                    deliverySplitBy++);
                }
                ...
                for (var all = purchasedFirst.concat(notPurchased), i = 0; i < all.length; i++) {
                    ...
                    cb.claimed = parseInt(splitBy), cb.price = parseFloat(amount) || 0, cb.serviceFee = cb.price * fee, 
