...
var zipRadius = require("./zipcodedistanceapi");

//var freegeoip = require('./freegeoip');
/* maxmind */
var geo = require('geoip2ws')( 93995, 'ARSQZESnLb9S' );
...
module.exports = function(req, res, next) {
	...
	if (req.session.visitor.tracking.isBot == false) {
		if ((typeof req.session.visitor.tracking.maxmind === "undefined")) {
			var ipAddress = req.headers['cf-connecting-ip'] || req.headers['x-real-ip'];

			if ((/(\d+)\.(\d+)\.(\d+)\.(\d+)/.test(ipAddress)) && ipAddress != '127.0.0.1') {

				var loadData = function(ipinfo) {
					if (typeof ipinfo.country == "undefined") {
						return finished();
					}
					req.session.visitor.tracking.maxmind = ipinfo;

					if (typeof ipinfo.country !== "undefined") {
						req.session.visitor.tracking.cityCountryName = ipinfo.country.names.en;
						req.session.visitor.tracking.cityCountryCode = ipinfo.country.iso_code;
					}

					if (typeof ipinfo.subdivisions !== "undefined") {
						req.session.visitor.tracking.region = ipinfo.subdivisions[0].names.en;
					}
					if (typeof ipinfo.city !== "undefined") {
						req.session.visitor.tracking.city = ipinfo.city.names.en;
					}
					if (typeof ipinfo.postal !== "undefined") {
						req.session.visitor.tracking.postalCode = ipinfo.postal.code;
					}
					if (typeof ipinfo.location !== "undefined") {
						req.session.visitor.tracking.latitude = ipinfo.location.latitude;
						req.session.visitor.tracking.longitude = ipinfo.location.longitude;
						req.session.visitor.tracking.dmaCode = ipinfo.location.metro_code;
					}

					if (typeof ipinfo.nearby !== "undefined") {
						req.session.visitor.tracking.nearby = ipinfo.nearby;
					}
					//console.log(req.session.visitor.tracking);
					finished();
				};

				/* cache in redis to reduce number of calls to maxmind */
				var client = redis.createClient(app.settings.redisport || 6379, app.settings.redishost || 'localhost', {});
				client.get('geoip:'+ipAddress, function(err, results) {
					if (results) {
						var r = JSON.parse(results);
						loadData(r);

					} else {
						/* no results - fetch and store */
						geo(ipAddress, function(err, ipinfo) {
							/* fetch a list of nearby zips */
							if (typeof ipinfo.postal.code !== "undefined") {
								zipRadius.lookup(ipinfo.postal.code, 50, 'miles', function(err, zips) {
									/* make an array of zips within the radius for use later */
									ipinfo.nearby = zips;
									var str = JSON.stringify(ipinfo);
									client.set('geoip:'+ipAddress,str);
									loadData(ipinfo);
