	...
	init: function(ea) {
		...
		ea.everymodule.findUserById(function(req, id, callback) {
			...
			Customer.findOne({"socialProfiles.facebook.id":id}, function(err, c) {
				...
				console.log('everyauth found customer');
				console.log(c);
				if (c !== null) {
		...
		var redirectUrl = (app.settings.host == 'tom') ? 'http://tom.wembli.com/login/facebook' : 'https://www2.wembli.com/login/facebook';
		...
		ea.twitter.consumerKey(conf.twit.appId).consumerSecret(conf.twit.appSecret).callbackPath('/auth/twitter/callback').handleAuthCallbackError(function(req, res) {
			...
			if (session.customer) {
				/* just save their profile in the cust obj and gtfo */
				Customer.update({email:session.email},{"socialProfiles.twitter":user},function(err,rows) {
					/* TODO: log errors */
		 ...
		 */
			...
			if (session.customer) {
				/* just save their profile in the cust obj and gtfo */
				Customer.update({email:session.email},{"socialProfiles.facebook":user},function(err,rows) {
					/* TODO: log errors */
					session.loggedIn = true;
					promise.fulfill(user);
				});
