...
module.exports = {
	SearchEvents: function(args, callback) {
		if (typeof args.searchTerms !== "undefined") {
			/* tn seems to do better when you UC the search terms */
			args.searchTerms = args.searchTerms.toUpperCase();
		}
		var template = {
			method: 'SearchEvents',
			searchTerms: '*',
	...
	/* this doesn't work :( */
	GetShippingMethods: function(args, callback) {
		var template = {
	...
	GetPricingInfo: function(args, callback) {
		var template = {
...
function makeRequest(args, callback) {
	...
	var base = '/tnwebservice/v3.2/tnwebservicestringinputs.asmx';
	var uri = base + '/' + args.method + '?WebsiteConfigId=8582&' + params;
	var host = 'tnwebservices.ticketnetwork.com';

	if (app.settings.tnUrl) {
		host = app.settings.tnUrl;
	}
