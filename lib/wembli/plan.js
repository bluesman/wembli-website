...
module.exports = function(req, res, next) {
	...
	var claimPlan = function(p) {
		...
		if (typeof req.session.customer !== "undefined") {
			...
			if (typeof req.session.plan.event.eventId !== "undefined") {
				console.log('plan has an eventid - saving it');
				req.session.plan.save(function(err) {
					...
					req.session.customer.addPlan(req.session.plan.guid, function(err, results) {
						console.log('adding plan to this customer');
			...
			console.log('cant claim yet cause theres no customer');
	...
	Plan.findOne({
		...
		console.log('this plan is in the db am i the organizer?');
		/* am I the organizer? */
		if (typeof p.organizer.customerId === "undefined") {
			console.log('have a plan in db and theres no organizer - i must be the organizer');
			return claimPlan(p);
		}
