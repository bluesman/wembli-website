...
module.exports = function(app) {
	...
	app.post(/\/signup\/?/, function(req, res) {
		if (!req.param('email')) {
			res.locals.errors.push('Please make sure all the form items are filled.');
		...
		if (req.param('password') != req.param('password2')) {
			res.locals.errors.push('The passwords do not match.');
			return res.redirect('/signup');
		}
		...
		Customer.findOne({
			...
			if (c == null) {
				...
				res.locals.errors.push('An account with that email exists.');
				return res.redirect('/signup');
			}
