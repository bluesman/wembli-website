(function(d) {
	var f = {
		...
		ShowTooltip: function(k, j, n) {
			...
			if (f.Settings.EnableTooltipSectionView) {
				...
				var g = function(o) {
					...
					f.Tooltip.animate({
						left: (d(window).outerWidth(true) - f.SectionViewSize) / 2,
						top: (d(window).outerHeight(true) - (f.SectionViewSize / 2)) / 2
					}, "fast").css("position", "fixed");
					o.parent().animate({
		...
		Initialize: function(h) {
			...
			var i = "Version=3.1.11&MapSet=" + f.Settings.MapSet + "&MapType=" + f.Settings.MapType + "&EnableGroups=" + (f.Settings.GroupsContainer != null).toString() + "&Domain=" + window.location.host + "&AutoSwitchToStatic=" + f.Settings.AutoSwitchToStatic;
			if (f.Settings.MapId == null && f.Settings.EventId == null) {
	...
	var e = {
		...
		Initialize: function(i) {
			...
			e.Wrapper = d("<div>").attr("style", "z-index:0;position:absolute;width:100%;height:100%").disableSelection().appendTo(e.Container).mousedown(function(o) {
				e.MouseDown = true;
